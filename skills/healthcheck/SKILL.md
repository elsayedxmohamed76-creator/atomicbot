---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AtomicBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AtomicBot cron scheduling for periodic checks, or version status checks on a machine running AtomicBot (laptop, workstation, Pi, VPS).
---

# AtomicBot Host Hardening

## Overview

Assess and harden the host running AtomicBot, then align it to a user-defined risk tolerance without breaking access. Use AtomicBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AtomicBot changes the host firewall, SSH, or OS updates; it does not.
...
5. AtomicBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AtomicBot can access sensitive data.
...
### 2) Run AtomicBot security audits (read-only)

As part of the default read-only checks, run `atomicbot security audit --deep`. Only offer alternatives if the user requests them:

1. `atomicbot security audit` (faster, non-probing)
2. `atomicbot security audit --json` (structured output)

Offer to apply AtomicBot safe defaults (numbered):

1. `atomicbot security audit --fix`

Be explicit that `--fix` only tightens AtomicBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AtomicBot version/update status (read-only)

As part of the default read-only checks, run `atomicbot update status`.
...
- Credential hygiene notes (location of AtomicBot creds, prefer disk encryption)
...
- AtomicBot security audit (re-run)
...
After AtomicBot install or first hardening pass, run at least one baseline audit and version check:

- `atomicbot security audit`
- `atomicbot security audit --deep`
- `atomicbot update status`

Ongoing monitoring is recommended. Use the AtomicBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `atomicbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `atomicbot update status`
...
Before creating, `atomicbot cron list` and match on exact `name`. If found, `atomicbot cron edit <id> ...`.
If not found, `atomicbot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `atomicbot update status` (preferred for source checkouts and channels)
2. `npm view atomicbot version` (published npm version)

## AtomicBot command accuracy

Use only supported commands and flags:

- `atomicbot security audit [--deep] [--fix] [--json]`
- `atomicbot status` / `atomicbot status --deep`
- `atomicbot health --json`
- `atomicbot update status`
- `atomicbot cron add|list|runs|run`

Do not invent CLI flags or imply AtomicBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AtomicBot compaction:
