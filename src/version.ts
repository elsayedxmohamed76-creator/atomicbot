...
declare const __ATOMICBOT_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "atomicbot";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ATOMICBOT_VERSION"],
      env["ATOMICBOT_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current AtomicBot version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __ATOMICBOT_VERSION__ === "string" && __ATOMICBOT_VERSION__) ||
  process.env.ATOMICBOT_BUNDLED_VERSION ||
