...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("atomicbot-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("atomicbot-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("atomicbot-lid-a-", (first) => {
      withTempDirSync("atomicbot-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.atomicbot when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "atomicbot-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ATOMICBOT_HOME over HOME", () => {
    vi.stubEnv("ATOMICBOT_HOME", "/srv/atomicbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/atomicbot-home"));
...
describe("shortenHomePath", () => {
  it("uses $ATOMICBOT_HOME prefix when ATOMICBOT_HOME is set", () => {
    vi.stubEnv("ATOMICBOT_HOME", "/srv/atomicbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/atomicbot-home")}/.atomicbot/atomicbot.json`)).toBe(
      "$ATOMICBOT_HOME/.atomicbot/atomicbot.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ATOMICBOT_HOME replacement when ATOMICBOT_HOME is set", () => {
    vi.stubEnv("ATOMICBOT_HOME", "/srv/atomicbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/atomicbot-home")}/.atomicbot/atomicbot.json`),
    ).toBe("config: $ATOMICBOT_HOME/.atomicbot/atomicbot.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/atomicbot")).toBe(path.resolve(os.homedir(), "atomicbot"));
  });
  ...
  it("prefers ATOMICBOT_HOME for tilde expansion", () => {
    vi.stubEnv("ATOMICBOT_HOME", "/srv/atomicbot-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/atomicbot")).toBe(path.resolve("/srv/atomicbot-home", "atomicbot"));
