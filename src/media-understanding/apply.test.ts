...
const TEMP_MEDIA_PREFIX = "atomicbot-media-";
...
function createGroqAudioConfig(): AtomicBotConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AtomicBotConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AtomicBotConfig {
  return {
