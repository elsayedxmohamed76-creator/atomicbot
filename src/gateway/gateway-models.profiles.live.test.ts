...
import { resolveAtomicBotAgentDir } from "../agents/agent-paths.js";
...
import { ensureAtomicBotModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, AtomicBotConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ATOMICBOT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ATOMICBOT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ATOMICBOT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: AtomicBotConfig;
  agentDir: string;
}): AtomicBotConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<AtomicBotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ATOMICBOT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ATOMICBOT_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveAtomicBotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-live-state-"));
  process.env.ATOMICBOT_STATE_DIR = tempStateDir;
  ...
  process.env.ATOMICBOT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.atomicbot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAtomicBotAgentDir();
  const sanitizedCfg: AtomicBotConfig = {
    ...params.cfg,
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "AtomicBot live tool probe (local, safe): " +
    ...
    process.env.ATOMICBOT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ATOMICBOT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAtomicBotModelsJson(cfg);
      ...
      const rawModels = process.env.ATOMICBOT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.atomicbot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
