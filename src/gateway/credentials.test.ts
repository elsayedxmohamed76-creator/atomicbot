...
function cfg(input: Partial<AtomicBotConfig>): AtomicBotConfig {
  return input as AtomicBotConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<AtomicBotConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  ATOMICBOT_GATEWAY_TOKEN: "env-token",
  ATOMICBOT_GATEWAY_PASSWORD: "env-password",
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy ATOMICBOT env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        ATOMICBOT_GATEWAY_TOKEN: "legacy-token",
        ATOMICBOT_GATEWAY_PASSWORD: "legacy-password",
      } as NodeJS.ProcessEnv,
