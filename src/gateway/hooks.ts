...
export function resolveHooksConfig(cfg: AtomicBotConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AtomicBotConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-atomicbot-token"] === "string"
      ? req.headers["x-atomicbot-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
