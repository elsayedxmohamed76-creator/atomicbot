...
beforeAll(async () => {
  previousToken = process.env.ATOMICBOT_GATEWAY_TOKEN;
  process.env.ATOMICBOT_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.ATOMICBOT_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "atomicbot-ios", platform: "ios" },
  { clientId: "atomicbot-android", platform: "android" },
