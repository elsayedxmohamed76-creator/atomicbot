...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ATOMICBOT_SKIP_BROWSER_CONTROL_SERVER",
  "ATOMICBOT_SKIP_GMAIL_WATCHER",
  "ATOMICBOT_SKIP_CANVAS_HOST",
  "ATOMICBOT_BUNDLED_PLUGINS_DIR",
  "ATOMICBOT_SKIP_CHANNELS",
  "ATOMICBOT_SKIP_PROVIDERS",
  "ATOMICBOT_SKIP_CRON",
  "ATOMICBOT_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.ATOMICBOT_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ATOMICBOT_SKIP_GMAIL_WATCHER = "1";
  process.env.ATOMICBOT_SKIP_CANVAS_HOST = "1";
  process.env.ATOMICBOT_SKIP_CHANNELS = "1";
  process.env.ATOMICBOT_SKIP_PROVIDERS = "1";
  process.env.ATOMICBOT_SKIP_CRON = "1";
  process.env.ATOMICBOT_TEST_MINIMAL_GATEWAY = "1";
  process.env.ATOMICBOT_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "atomicbot-test-no-bundled-extensions")
    : "atomicbot-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "atomicbot-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".atomicbot-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__atomicbotTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__atomicbotTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ATOMICBOT_GATEWAY_TOKEN"]);
  const prev = process.env.ATOMICBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ATOMICBOT_GATEWAY_TOKEN = fallbackToken;
  }
