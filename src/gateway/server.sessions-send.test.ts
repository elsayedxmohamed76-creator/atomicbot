...
const { createAtomicBotTools } = await import("../agents/atomicbot-tools.js");
...
type SessionSendTool = ReturnType<typeof createAtomicBotTools>[number];

function getSessionsSendTool(): SessionSendTool {
  const tool = createAtomicBotTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ATOMICBOT_GATEWAY_PORT", "ATOMICBOT_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ATOMICBOT_GATEWAY_PORT = String(gatewayPort);
  process.env.ATOMICBOT_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    const configPath = process.env.ATOMICBOT_CONFIG_PATH;
    if (!configPath) {
      throw new Error("ATOMICBOT_CONFIG_PATH missing in gateway test environment");
    }
