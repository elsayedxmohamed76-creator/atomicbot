...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("atomicbot doctor --fix");
  });
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      label: "password",
      authKey: "password",
      envKey: "ATOMICBOT_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      label: "token",
      authKey: "token",
      envKey: "ATOMICBOT_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ATOMICBOT_GATEWAY_PASSWORD", "ATOMICBOT_GATEWAY_TOKEN"]);
    resetGatewayCallMocks();
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ATOMICBOT_GATEWAY_PASSWORD = envPassword;
    }
