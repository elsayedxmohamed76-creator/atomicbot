...
import type { AtomicBotConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: AtomicBotConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: AtomicBotConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: AtomicBotConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: AtomicBotConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: AtomicBotConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: AtomicBotConfig) => {
