...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.ATOMICBOT_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.ATOMICBOT_GATEWAY_TOKEN);
}
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.ATOMICBOT_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.ATOMICBOT_GATEWAY_PASSWORD);
}
