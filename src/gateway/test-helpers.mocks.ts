...
type GetReplyFromConfigFn = (
  ...
  configOverride?: AtomicBotConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `atomicbot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ATOMICBOT_CONFIG_PATH = path.join(root, "atomicbot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "atomicbot.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "atomicbot-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadAtomicBotPlugins: () => pluginRegistryState.registry,
  };
});
