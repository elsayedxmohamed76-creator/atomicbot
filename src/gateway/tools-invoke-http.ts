...
import { createAtomicBotTools } from "../agents/atomicbot-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-atomicbot-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-atomicbot-account-id")?.trim() || undefined;
  ...
  const allTools = createAtomicBotTools({
