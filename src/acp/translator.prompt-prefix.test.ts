...
describe("acp prompt cwd prefix", () => {
  async function runPromptWithCwd(cwd: string) {
    ...
    const previousAtomicBotHome = process.env.ATOMICBOT_HOME;
    ...
    try {
      ...
      if (previousAtomicBotHome === undefined) {
        delete process.env.ATOMICBOT_HOME;
      } else {
        process.env.ATOMICBOT_HOME = previousAtomicBotHome;
      }
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "atomicbot-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]atomicbot-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\atomicbot-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\atomicbot-test]"),
      }),
      { expectFinal: true },
    );
  });
