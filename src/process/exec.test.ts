...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ ATOMICBOT_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.ATOMICBOT_BASE_ENV ?? "") + "|" + (process.env.ATOMICBOT_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { ATOMICBOT_TEST_ENV: "ok" },
        },
      );
