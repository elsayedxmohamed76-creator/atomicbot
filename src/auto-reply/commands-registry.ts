...
export function isCommandEnabled(cfg: AtomicBotConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AtomicBotConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AtomicBotConfig),
...
export function getCommandDetection(_cfg?: AtomicBotConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AtomicBotConfig) {
