...
export async function prependSystemEvents(params: {
  ...
  const resolveSystemEventTimezone = (cfg: AtomicBotConfig) => {
  ...
  const formatSystemEventTimestamp = (ts: number, cfg: AtomicBotConfig) => {
...
export async function ensureSkillSnapshot(params: {
  ...
  if (process.env.ATOMICBOT_TEST_FAST === "1") {
