...
import { scheduleGatewaySigusr1Restart, triggerAtomicBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AtomicBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAtomicBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AtomicBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
