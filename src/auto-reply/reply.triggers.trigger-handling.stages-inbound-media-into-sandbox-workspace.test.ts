...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("atomicbot-triggers-", async (home) => {
      const inboundDir = join(home, ".atomicbot", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "atomicbot"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("atomicbot-triggers-bypass-", async (home) => {
      // Sensitive host file outside .atomicbot
  ...
  it("blocks remote SCP staging for non-iMessage attachment paths", async () => {
    await withSandboxMediaTempHome("atomicbot-triggers-remote-block-", async (home) => {
      const sandboxDir = join(home, "sandboxes", "session");
      vi.mocked(ensureSandboxWorkspaceForSession).mockResolvedValue({
        workspaceDir: sandboxDir,
        containerWorkdir: "/work",
