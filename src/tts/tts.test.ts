...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ATOMICBOT_TTS_PREFS;
      process.env.ATOMICBOT_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.ATOMICBOT_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: AtomicBotConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
