...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "atomicbot.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "atomicbot.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ATOMICBOT_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ATOMICBOT_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ATOMICBOT_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[atomicbot] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[atomicbot] ${parsed.error}`);
      process.exit(2);
    }
