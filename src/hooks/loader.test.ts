...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ATOMICBOT_BUNDLED_HOOKS_DIR"]);
    process.env.ATOMICBOT_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
