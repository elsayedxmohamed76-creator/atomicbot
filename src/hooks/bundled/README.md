...
This directory contains hooks that ship with AtomicBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.atomicbot/atomicbot.json`:
...
2. Restart gateway: `pkill -9 -f 'atomicbot.*gateway' && pnpm atomicbot gateway`
3. Enable the hook: `atomicbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.atomicbot.ai/automation/hooks
