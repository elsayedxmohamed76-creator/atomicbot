...
homepage: https://docs.atomicbot.ai/automation/hooks#command-logger
metadata:
  {
    "atomicbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.atomicbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.atomicbot/logs/commands.log ~/.atomicbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/atomicbot`:
   ```
   /home/username/.atomicbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
