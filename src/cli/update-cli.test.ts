...
import type { AtomicBotConfig, ConfigFileSnapshot } from "../config/types.atomicbot.js";
...
vi.mock("../infra/atomicbot-root.js", () => ({
  resolveAtomicBotPackageRoot: vi.fn(),
}));
...
const { resolveAtomicBotPackageRoot } = await import("../infra/atomicbot-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/atomicbot-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as AtomicBotConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/atomicbot-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveAtomicBotPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("atomicbot-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveAtomicBotPackageRoot).mockClear();
    ...
    vi.mocked(resolveAtomicBotPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("atomicbot");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/atomicbot-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "atomicbot-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("AtomicBot update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as AtomicBotConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveAtomicBotPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("atomicbot-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ATOMICBOT_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ATOMICBOT_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("atomicbot-update-wizard");
    await withEnvAsync({ ATOMICBOT_GIT_DIR: tempDir }, async () => {
      setTty(true);
