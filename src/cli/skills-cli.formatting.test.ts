...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "atomicbot-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "atomicbot-bundled-skills-test-"));
    process.env.ATOMICBOT_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
