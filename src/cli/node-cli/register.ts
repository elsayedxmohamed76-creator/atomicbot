    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "atomicbot node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["atomicbot node status", "Check node host service status."],
          ["atomicbot node install", "Install the node host service."],
          ["atomicbot node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.atomicbot.ai/cli/node")}\n`,
    );
