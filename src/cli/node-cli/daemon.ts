...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("atomicbot node install"), formatCliCommand("atomicbot node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("atomicbot node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ATOMICBOT_LOG_PREFIX: baseEnv.ATOMICBOT_LOG_PREFIX ?? "node",
