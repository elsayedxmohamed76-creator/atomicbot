...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# AtomicBot Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(atomicbot completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# AtomicBot Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(atomicbot completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.atomicbot.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $ATOMICBOT_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "atomicbot") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c atomicbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'atomicbot gateway' and not 'atomicbot other gateway', we need to check the sequence.
