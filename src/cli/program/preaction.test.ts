...
beforeEach(() => {
  ...
  originalHideBanner = process.env.ATOMICBOT_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.ATOMICBOT_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.ATOMICBOT_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("atomicbot");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "atomicbot", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("atomicbot-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "atomicbot", "message", "send"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "atomicbot", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "atomicbot", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "atomicbot", "--version"],
    });
  ...
  it("hides banner when ATOMICBOT_HIDE_BANNER is truthy", async () => {
    process.env.ATOMICBOT_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "atomicbot", "status"],
    });
