...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "atomicbot", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "atomicbot", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "atomicbot", "status"]);
  });
