...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.ATOMICBOT_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.ATOMICBOT_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<AtomicBotConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live AtomicBot docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  {
    name: "plugins",
    description: "Manage AtomicBot plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
