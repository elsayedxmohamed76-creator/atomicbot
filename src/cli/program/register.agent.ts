...
${formatHelpExamples([
  ['atomicbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['atomicbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'atomicbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'atomicbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['atomicbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'atomicbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.atomicbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.atomicbot.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['atomicbot agents set-identity --agent main --name "AtomicBot" --emoji "🦞"', "Set name + emoji."],
  ["atomicbot agents set-identity --agent main --avatar avatars/atomicbot.png", "Set avatar path."],
  [
    "atomicbot agents set-identity --workspace ~/.atomicbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "atomicbot agents set-identity --identity-file ~/.atomicbot/workspace/IDENTITY.md --agent main",
