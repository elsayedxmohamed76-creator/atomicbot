...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.atomicbot.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AtomicBot hooks")
    ...
    .option("--hook-url <url>", "AtomicBot hook URL")
    .option("--hook-token <token>", "AtomicBot hook token")
