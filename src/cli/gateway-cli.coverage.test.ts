...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio atomicbot.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.atomicbot.internal",
        "  ws: ws://studio.atomicbot.internal:18789",
      ],
    },
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ ATOMICBOT_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
