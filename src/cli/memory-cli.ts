    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["atomicbot memory status", "Show index and provider status."],
          ["atomicbot memory index --force", "Force a full reindex."],
          ['atomicbot memory search --query "deployment notes"', "Search indexed memory entries."],
          ["atomicbot memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.atomicbot.ai/cli/memory")}\n`,
    );
