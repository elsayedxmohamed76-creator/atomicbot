...
import { resolveAtomicBotPackageRoot } from "../../infra/atomicbot-root.js";
...
const ATOMICBOT_REPO_URL = "https://github.com/atomicbot/atomicbot.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "atomicbot";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("atomicbot@")) {
    return trimmed.slice("atomicbot@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.ATOMICBOT_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveAtomicBotPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ATOMICBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ATOMICBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set ATOMICBOT_GIT_DIR to an empty folder or an atomicbot checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ATOMICBOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "atomicbot.mjs");
