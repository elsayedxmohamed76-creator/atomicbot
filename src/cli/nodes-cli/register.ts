    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["atomicbot nodes status", "List known nodes with live status."],
          ["atomicbot nodes pairing pending", "Show pending node pairing requests."],
          ['atomicbot nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["atomicbot nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.atomicbot.ai/cli/nodes")}\n`,
    );
