...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@atomicbot/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@atomicbot/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@atomicbot/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@atomicbot/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @atomicbot/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@atomicbot/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@atomicbot/plugin-alpha@1.2.3",
        installPath: "/tmp/atomicbot/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@atomicbot/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @atomicbot/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@atomicbot/plugin-alpha@latest",
      true,
      "/tmp/atomicbot/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@atomicbot/plugin-alpha@latest",
      installPath: "/tmp/atomicbot/extensions/alpha",
      version: "1.2.3",
