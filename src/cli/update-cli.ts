...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AtomicBot and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["atomicbot update", "Update a source checkout (git)"],
        ["atomicbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["atomicbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["atomicbot update --tag beta", "One-off update to a dist-tag or version"],
        ["atomicbot update --dry-run", "Preview actions without changing anything"],
        ["atomicbot update --no-restart", "Update without restarting the service"],
        ["atomicbot update --json", "Output result as JSON"],
        ["atomicbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["atomicbot update wizard", "Interactive update wizard"],
        ["atomicbot --update", "Shorthand for atomicbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run atomicbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.atomicbot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["atomicbot update status", "Show channel + version status."],
          ["atomicbot update status --json", "JSON output."],
          ["atomicbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.atomicbot.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
