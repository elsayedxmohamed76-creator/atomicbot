...
const DEFAULT_PACKAGE_NAME = "atomicbot";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "atomicbot.mjs");
    ...
    const doctorStep = await runStep(
      step("atomicbot doctor", doctorArgv, gitRoot, { ATOMICBOT_UPDATE_IN_PROGRESS: "1" }),
    );
