...
vi.mock("./atomicbot-root.js", () => ({
  resolveAtomicBotPackageRoot: vi.fn(async () => null),
  resolveAtomicBotPackageRootSync: vi.fn(() => null),
}));
...
let atomicbotRoot: typeof import("./atomicbot-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    atomicbotRoot = await import("./atomicbot-root.js");
  });
  ...
  it("uses resolveAtomicBotPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/atomicbot");
    (
      atomicbotRoot.resolveAtomicBotPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/atomicbot"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "atomicbot" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "atomicbot.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-atomicbot");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/atomicbot-bundle");
    (
      atomicbotRoot.resolveAtomicBotPackageRootSync as unknown as ReturnType<typeof vi.fn>
