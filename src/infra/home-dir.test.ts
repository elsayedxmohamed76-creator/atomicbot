...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ATOMICBOT_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/atomicbot-home"),
    );
  });
  ...
  it("expands ATOMICBOT_HOME when set to ~", () => {
    const env = {
      ATOMICBOT_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ATOMICBOT_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ATOMICBOT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ATOMICBOT_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ATOMICBOT_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ATOMICBOT_HOME: "/srv/atomicbot-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/atomicbot-home")}/x`);
  });
