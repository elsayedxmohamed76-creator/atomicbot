...
import { POSIX_ATOMICBOT_TMP_DIR, resolvePreferredAtomicBotTmpDir } from "./tmp-atomicbot-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAtomicBotTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `atomicbot-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredAtomicBotTmpDir({
    accessSync,
...
describe("resolvePreferredAtomicBotTmpDir", () => {
  it("prefers /tmp/atomicbot when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ATOMICBOT_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/atomicbot when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ATOMICBOT_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/atomicbot when /tmp/atomicbot is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/atomicbot when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/atomicbot is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/atomicbot is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/atomicbot is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
