...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AtomicBotConfig | null = null;
  ...
  get(key: string, cfg: AtomicBotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AtomicBotConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: AtomicBotConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: AtomicBotConfig): void {
