...
function candidateBinDirs(opts: EnsureAtomicBotPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `atomicbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "atomicbot");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ATOMICBOT_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "atomicbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `atomicbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAtomicBotCliOnPath(opts: EnsureAtomicBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ATOMICBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
