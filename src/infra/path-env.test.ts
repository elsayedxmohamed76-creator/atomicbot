...
let ensureAtomicBotCliOnPath: typeof import("./path-env.js").ensureAtomicBotCliOnPath;

describe("ensureAtomicBotCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ATOMICBOT_PATH_BOOTSTRAPPED",
    "ATOMICBOT_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAtomicBotCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling atomicbot exists", () => {
    const tmp = abs("/tmp/atomicbot-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "atomicbot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ATOMICBOT_PATH_BOOTSTRAPPED;

    ensureAtomicBotCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ATOMICBOT_PATH_BOOTSTRAPPED = "1";
    ensureAtomicBotCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/atomicbot-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "atomicbot");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/atomicbot-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "atomicbot");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "atomicbot");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/atomicbot-path/case-linuxbrew");
