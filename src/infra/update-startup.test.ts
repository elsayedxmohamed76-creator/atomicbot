...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ATOMICBOT_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ATOMICBOT_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAtomicBotPackageRoot } = await import("./atomicbot-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveAtomicBotPackageRoot).mockResolvedValue("/opt/atomicbot");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/atomicbot/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
