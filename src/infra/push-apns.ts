...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ATOMICBOT_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ATOMICBOT_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ATOMICBOT_APNS_TEAM_ID and ATOMICBOT_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ATOMICBOT_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ATOMICBOT_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ATOMICBOT_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ATOMICBOT_APNS_PRIVATE_KEY_P8 or ATOMICBOT_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ATOMICBOT_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createAtomicBotPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    atomicbot: createAtomicBotPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
