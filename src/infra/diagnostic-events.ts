...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __atomicbotDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__atomicbotDiagnosticEventsState) {
    globalStore.__atomicbotDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__atomicbotDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: AtomicBotConfig): boolean {
