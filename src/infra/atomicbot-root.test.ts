...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__atomicbot_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "atomicbot-root");
...
describe("resolveAtomicBotPackageRoot", () => {
  let resolveAtomicBotPackageRoot: typeof import("./atomicbot-root.js").resolveAtomicBotPackageRoot;
  let resolveAtomicBotPackageRootSync: typeof import("./atomicbot-root.js").resolveAtomicBotPackageRootSync;

  beforeAll(async () => {
    ({ resolveAtomicBotPackageRoot, resolveAtomicBotPackageRootSync } =
      await import("./atomicbot-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "atomicbot");
    const pkgRoot = path.join(project, "node_modules", "atomicbot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "atomicbot" }));

    expect(resolveAtomicBotPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "atomicbot");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "atomicbot.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "atomicbot" }));

    expect(resolveAtomicBotPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAtomicBotPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-atomicbot package roots", async () => {
    const pkgRoot = fx("not-atomicbot");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-atomicbot" }));

    expect(resolveAtomicBotPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveAtomicBotPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveAtomicBotPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
