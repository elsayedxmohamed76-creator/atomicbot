...
export function triggerAtomicBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ATOMICBOT_SYSTEMD_UNIT,
        process.env.ATOMICBOT_PROFILE,
      );
  ...
  const label =
    process.env.ATOMICBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ATOMICBOT_PROFILE);
