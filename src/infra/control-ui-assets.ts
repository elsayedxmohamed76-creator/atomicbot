...
import { resolveAtomicBotPackageRoot, resolveAtomicBotPackageRootSync } from "./atomicbot-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAtomicBotPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "atomicbot" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAtomicBotPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // atomicbot.mjs or dist/<bundle>.js
