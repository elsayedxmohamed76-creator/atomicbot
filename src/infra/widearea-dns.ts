...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ATOMICBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*atomicbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "atomicbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "atomicbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "atomicbot-gw");
  ...
  records.push(`_atomicbot-gw._tcp IN PTR ${instanceLabel}._atomicbot-gw._tcp`);
  records.push(`${instanceLabel}._atomicbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._atomicbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; atomicbot-content-hash: ${contentHash}\n${contentBody}`;
}
