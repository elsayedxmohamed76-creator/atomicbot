...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ATOMICBOT_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "AtomicBot";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(AtomicBot\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.ATOMICBOT_MDNS_HOSTNAME?.trim() ||
    process.env.ATOMICBOT_MDNS_HOSTNAME?.trim() ||
    "atomicbot";
  const hostname =
    hostnameRaw
