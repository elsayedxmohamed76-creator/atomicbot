...
import type { AtomicBotConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredAtomicBotTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "atomicbot.log"); // legacy single-file path

const LOG_PREFIX = "atomicbot";
...
function resolveSettings(): ResolvedSettings {
  let cfg: AtomicBotConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => AtomicBotConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ATOMICBOT_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[atomicbot] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
