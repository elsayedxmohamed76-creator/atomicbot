...
import { loadAtomicBotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `atomicbot-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.ATOMICBOT_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "atomicbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.ATOMICBOT_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadAtomicBotPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.ATOMICBOT_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ATOMICBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAtomicBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadAtomicBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAtomicBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.ATOMICBOT_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ ATOMICBOT_STATE_DIR: stateDir, ATOMICBOT_STATE_DIR: undefined }, () => {
