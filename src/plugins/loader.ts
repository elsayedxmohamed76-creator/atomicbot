...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AtomicBotPluginDefinition;
  register?: AtomicBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AtomicBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AtomicBotPluginDefinition;
...
export function loadAtomicBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAtomicBotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "atomicbot/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "atomicbot/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AtomicBotPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as AtomicBotPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
