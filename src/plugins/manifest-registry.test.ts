...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `atomicbot-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "atomicbot.plugin.json"), JSON.stringify(manifest), "utf-8");
}
