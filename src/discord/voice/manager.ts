...
function resolveVoiceTtsConfig(params: { cfg: AtomicBotConfig; override?: TtsConfig }): {
  cfg: AtomicBotConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredAtomicBotTmpDir(), "discord-voice-"));
