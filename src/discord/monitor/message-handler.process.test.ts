...
const resolveStorePath = vi.fn(() => "/tmp/atomicbot-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/atomicbot-discord-process-test-sessions.json");
  ...
  async function createBlockModeContext() {
    return await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/atomicbot-discord-process-test-sessions.json" },
        channels: {
          discord: {
