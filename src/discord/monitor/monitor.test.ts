...
describe("agent components", () => {
  const createCfg = (): AtomicBotConfig => ({}) as AtomicBotConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): AtomicBotConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/atomicbot-sessions-test.json");
  });
