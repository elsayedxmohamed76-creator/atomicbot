...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of AtomicBot")).toBe("friends-of-atomicbot");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of AtomicBot"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-atomicbot" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-atomicbot" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of AtomicBot"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-atomicbot");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-atomicbot": { slug: "friends-of-atomicbot" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of AtomicBot"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["atomicbot-dm"],
        channelId: "1",
        channelName: "AtomicBot DM",
        channelSlug: "atomicbot-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
