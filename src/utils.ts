...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".atomicbot");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ATOMICBOT_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ATOMICBOT_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ATOMICBOT_STATE_DIR.
