...
describe("canvas host state dir defaults", () => {
  it("uses ATOMICBOT_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("atomicbot-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("AtomicBot Canvas");
      } finally {
        await handler.close();
      }
    });
