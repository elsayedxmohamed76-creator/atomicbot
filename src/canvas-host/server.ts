...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.atomicbotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.atomicbotCanvasA2UIAction &&
        typeof window.atomicbotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.atomicbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("atomicbot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AtomicBot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.atomicbotSendUserAction === "function"
        ? window.atomicbotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ATOMICBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ATOMICBOT_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AtomicBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
