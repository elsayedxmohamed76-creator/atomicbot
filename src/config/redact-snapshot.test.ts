...
import type { ConfigFileSnapshot } from "./types.atomicbot.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.atomicbot/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/atomicbot/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/atomicbot/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/atomicbot/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/atomicbot/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AtomicBotSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AtomicBotConfig";
    const hints = mapSensitivePaths(AtomicBotSchema, "", {});
