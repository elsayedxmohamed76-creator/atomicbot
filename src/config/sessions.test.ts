...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ATOMICBOT_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-atomicbot",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-atomicbot#general");
  });
  ...
  it("derives session transcripts dir from ATOMICBOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ATOMICBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.atomicbot");
  ...
  it("resolves cross-agent paths when ATOMICBOT_STATE_DIR differs from stored paths", () => {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.atomicbot"), () => {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
