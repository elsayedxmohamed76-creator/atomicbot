...
import type { AtomicBotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AtomicBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AtomicBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AtomicBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AtomicBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AtomicBotConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as AtomicBotConfig,
  };
}
