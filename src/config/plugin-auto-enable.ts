...
import type { AtomicBotConfig } from "./config.js";
...
function isTelegramConfigured(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isIrcConfigured(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: AtomicBotConfig): boolean {
...
function isIMessageConfigured(cfg: AtomicBotConfig): boolean {
...
function isWhatsAppConfigured(cfg: AtomicBotConfig): boolean {
...
function isGenericChannelConfigured(cfg: AtomicBotConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: AtomicBotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AtomicBotConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: AtomicBotConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AtomicBotConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AtomicBotConfig, pluginId: string): AtomicBotConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
