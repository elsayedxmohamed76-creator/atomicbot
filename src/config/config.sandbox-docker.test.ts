...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("atomicbot-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "atomicbot-sandbox-browser-global" },
      agentBrowser: { network: "atomicbot-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("atomicbot-sandbox-browser-agent");
  });
