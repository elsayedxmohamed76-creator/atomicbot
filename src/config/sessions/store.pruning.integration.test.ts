...
// Keep integration tests deterministic: never read a real atomicbot.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.ATOMICBOT_SESSION_CACHE_TTL_MS;
    process.env.ATOMICBOT_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.ATOMICBOT_SESSION_CACHE_TTL_MS;
    } else {
      process.env.ATOMICBOT_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
