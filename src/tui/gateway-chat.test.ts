...
describe("resolveGatewayConnection", () => {
  ...
  it("uses ATOMICBOT_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ATOMICBOT_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ATOMICBOT_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ATOMICBOT_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
