...
function listAgents(cfg: AtomicBotConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: AtomicBotConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AtomicBotConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AtomicBotConfig, EvaluatedBindingsCache>();
