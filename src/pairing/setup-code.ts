...
function resolveGatewayPort(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.ATOMICBOT_GATEWAY_PASSWORD?.trim() ||
    env.ATOMICBOT_GATEWAY_PASSWORD?.trim() ||
