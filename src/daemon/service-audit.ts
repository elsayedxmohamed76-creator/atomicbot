...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.ATOMICBOT_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service ATOMICBOT_GATEWAY_TOKEN does not match gateway.auth.token in atomicbot.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `atomicbot gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
