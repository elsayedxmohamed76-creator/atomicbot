...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ATOMICBOT_GATEWAY_PORT).toBe("18789");
    expect(env.ATOMICBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.ATOMICBOT_SERVICE_MARKER).toBe("atomicbot");
    expect(env.ATOMICBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ATOMICBOT_SERVICE_VERSION).toBe("string");
    expect(env.ATOMICBOT_SYSTEMD_UNIT).toBe("atomicbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ATOMICBOT_LAUNCHD_LABEL).toBe("ai.atomicbot.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ATOMICBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ATOMICBOT_SYSTEMD_UNIT).toBe("atomicbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ATOMICBOT_LAUNCHD_LABEL).toBe("ai.atomicbot.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".atomicbot"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ATOMICBOT_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".atomicbot-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ATOMICBOT_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".atomicbot"));
  });

  it("uses ATOMICBOT_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ATOMICBOT_STATE_DIR: "/var/lib/atomicbot" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/atomicbot"));
  });

  it("expands ~ in ATOMICBOT_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ATOMICBOT_STATE_DIR: "~/atomicbot-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/atomicbot-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ATOMICBOT_STATE_DIR: "C:\\State\\atomicbot" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\atomicbot");
  });
});
