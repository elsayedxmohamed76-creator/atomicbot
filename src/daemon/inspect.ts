...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "atomicbot" | "atomicbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["atomicbot", "atomicbot", "moltbot"] as const;

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.ATOMICBOT_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["atomicbot_service_marker"];
  const kindKeys = ["atomicbot_service_kind"];
...
function isAtomicBotGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.atomicbot.");
}

function isAtomicBotGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("atomicbot-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isAtomicBotGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("atomicbot gateway");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("atomicbot") || lower.includes("moltbot");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "atomicbot" : "moltbot",
        legacy: true,
      });
      continue;
    }
    ...
    if (marker === "atomicbot" && isAtomicBotGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "atomicbot" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "atomicbot" && isAtomicBotGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isAtomicBotGatewayTaskName(name)) {
        continue;
      }
