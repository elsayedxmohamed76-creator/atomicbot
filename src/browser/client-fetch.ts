...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-atomicbot-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-atomicbot-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-atomicbot-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the AtomicBot gateway (AtomicBot.app menubar, or \`${formatCliCommand("atomicbot gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the AtomicBot browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the AtomicBot browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
