...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".atomicbot-profile-decorated");
}
 ...
 */
export function decorateAtomicBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ATOMICBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ATOMICBOT_BROWSER_COLOR).toUpperCase();
