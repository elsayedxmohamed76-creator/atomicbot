import type { BrowserConfig, BrowserProfileConfig, AtomicBotConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ATOMICBOT_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "atomicbot" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_ATOMICBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "atomicbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ATOMICBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ATOMICBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AtomicBot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for atomicbot/atomicbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AtomicBotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ATOMICBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "atomicbot";
