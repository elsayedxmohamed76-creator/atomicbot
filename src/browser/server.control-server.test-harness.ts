...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ATOMICBOT_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/atomicbot" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "atomicbot",
        profiles: {
          atomicbot: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchAtomicBotChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.ATOMICBOT_GATEWAY_PORT;
  process.env.ATOMICBOT_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ATOMICBOT_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ATOMICBOT_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ATOMICBOT_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.ATOMICBOT_GATEWAY_PASSWORD;
  } else {
    process.env.ATOMICBOT_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
