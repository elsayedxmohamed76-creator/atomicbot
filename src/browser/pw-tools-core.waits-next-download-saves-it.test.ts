...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredAtomicBotTmpDir: vi.fn(() => "/tmp/atomicbot"),
}));
vi.mock("../infra/tmp-atomicbot-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredAtomicBotTmpDir.mockReturnValue("/tmp/atomicbot");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredAtomicBotTmpDir.mockReturnValue("/tmp/atomicbot-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "atomicbot-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredAtomicBotTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "atomicbot-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "atomicbot-preferred", "downloads")}${path.sep}`),
    );
  });
