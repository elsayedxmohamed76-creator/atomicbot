...
const RELAY_AUTH_HEADER = "x-atomicbot-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/AtomicBot-Extension-Relay",
          revision: "0",
          userAgent: "AtomicBot-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "AtomicBot/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedAtomicBotRelay({
