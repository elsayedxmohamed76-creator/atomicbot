  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          atomicbot: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ATOMICBOT_GATEWAY_PORT;
    process.env.ATOMICBOT_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ATOMICBOT_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ATOMICBOT_GATEWAY_PASSWORD;
