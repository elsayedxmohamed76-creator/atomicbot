...
vi.mock("./chrome.js", () => ({
  resolveAtomicBotUserDataDir: vi.fn(() => "/tmp/atomicbot-test/atomicbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveAtomicBotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "atomicbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAtomicBotUserDataDir).mockReturnValue(userDataDir);
