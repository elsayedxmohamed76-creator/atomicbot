...
const RELAY_TOKEN_CONTEXT = "atomicbot-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ATOMICBOT_RELAY_BROWSER = "AtomicBot/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.ATOMICBOT_GATEWAY_TOKEN?.trim() || process.env.ATOMICBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or ATOMICBOT_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedAtomicBotRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ATOMICBOT_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
