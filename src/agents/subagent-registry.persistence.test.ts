...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ATOMICBOT_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-subagent-"));
  ...
  it("uses isolated temp state when ATOMICBOT_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "atomicbot-test-state"));
  });
});
