...
describe("createAtomicBotCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-read-"));
    try {
      ...
      const contents = "Hello from atomicbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createAtomicBotCodingTools({ sandbox });
