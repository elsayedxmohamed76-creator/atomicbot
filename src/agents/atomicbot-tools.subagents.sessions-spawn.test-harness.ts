...
type CreateAtomicBotTools = (typeof import("./atomicbot-tools.js"))["createAtomicBotTools"];
export type CreateAtomicBotToolsOpts = Parameters<CreateAtomicBotTools>[0];
...
export async function getSessionsSpawnTool(opts: CreateAtomicBotToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createAtomicBotTools } = await import("./atomicbot-tools.js");
  const tool = createAtomicBotTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
