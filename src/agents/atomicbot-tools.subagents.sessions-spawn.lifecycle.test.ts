...
} from "./atomicbot-tools.subagents.sessions-spawn.test-harness.js";
...
describe("atomicbot-tools: subagents (sessions_spawn lifecycle)", () => {
  let previousFastTestEnv: string | undefined;

  beforeEach(() => {
    if (previousFastTestEnv === undefined) {
      previousFastTestEnv = process.env.ATOMICBOT_TEST_FAST;
    }
    vi.stubEnv("ATOMICBOT_TEST_FAST", "1");
    resetSessionsSpawnConfigOverride();
    setSessionsSpawnConfigOverride({
      session: {
        mainKey: "main",
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.ATOMICBOT_TEST_FAST;
      return;
    }
    process.env.ATOMICBOT_TEST_FAST = previousFastTestEnv;
  });
