...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "atomicbot.sandbox=1");
  args.push("--label", `atomicbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `atomicbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `atomicbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "atomicbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`atomicbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`atomicbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("atomicbot sandbox recreate --all");
}
