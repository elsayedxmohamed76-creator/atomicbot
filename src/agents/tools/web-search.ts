...
type WebSearchConfig = NonNullable<AtomicBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AtomicBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("atomicbot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.atomicbot.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://atomicbot.ai",
      "X-Title": "AtomicBot Web Search",
    },
