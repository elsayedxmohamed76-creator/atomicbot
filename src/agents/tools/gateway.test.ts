...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    atomicbot: process.env.ATOMICBOT_GATEWAY_TOKEN,
    atomicbot: process.env.ATOMICBOT_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.atomicbot === undefined) {
      delete process.env.ATOMICBOT_GATEWAY_TOKEN;
    } else {
      process.env.ATOMICBOT_GATEWAY_TOKEN = envSnapshot.atomicbot;
    }
    if (envSnapshot.atomicbot === undefined) {
      delete process.env.ATOMICBOT_GATEWAY_TOKEN;
    } else {
  ...
  it("uses ATOMICBOT_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ATOMICBOT_GATEWAY_TOKEN = "local-env-token";
    process.env.ATOMICBOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
