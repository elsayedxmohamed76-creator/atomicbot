...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AtomicBotConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AtomicBotConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AtomicBotConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AtomicBotConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: AtomicBotConfig, accountId = "media") {
    await handleTelegramAction(
