...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("atomicbot-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("atomicbot-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("atomicbot-sbx-");
