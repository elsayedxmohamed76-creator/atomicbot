...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureAtomicBotModelsJson(validated.config);

      const modelPath = path.join(resolveAtomicBotAgentDir(), "models.json");
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureAtomicBotModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
