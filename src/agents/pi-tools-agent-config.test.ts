...
import { createAtomicBotCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: AtomicBotConfig) {
    return createAtomicBotCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<AtomicBotConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<AtomicBotConfig["agents"]>["list"]>[number]["tools"];
  }): AtomicBotConfig {
    return {
      tools: params.tools,
      agents: {
    ...
    const cfg: AtomicBotConfig = {
      ...
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/atomicbot-restricted",
            tools: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAtomicBotCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAtomicBotCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAtomicBotCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAtomicBotCodingTools({
      config: cfg,
    ...
    const bobTools = createAtomicBotCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAtomicBotCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAtomicBotCodingTools({
      config: cfg,
