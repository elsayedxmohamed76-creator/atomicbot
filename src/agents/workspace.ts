...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ATOMICBOT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".atomicbot", `workspace-${profile}`);
  }
  return path.join(home, ".atomicbot", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".atomicbot";
