...
const GOOGLE_LIVE_SINGLE_KEY = "ATOMICBOT_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ATOMICBOT_LIVE_ANTHROPIC_KEY",
    listVar: "ATOMICBOT_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ATOMICBOT_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ATOMICBOT_LIVE_${base}_KEY`;
