...
function resolveSkillsLimits(config?: AtomicBotConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "atomicbot-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "atomicbot-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveAtomicBotMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: AtomicBotConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`atomicbot skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
