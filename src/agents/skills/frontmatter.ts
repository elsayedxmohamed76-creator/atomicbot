...
import {
  ...
  normalizeStringList,
  parseAtomicBotManifestInstallBase,
  parseFrontmatterBool,
  resolveAtomicBotManifestBlock,
  resolveAtomicBotManifestInstall,
  resolveAtomicBotManifestOs,
  resolveAtomicBotManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  AtomicBotSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseAtomicBotManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveAtomicBotMetadata(
  frontmatter: ParsedSkillFrontmatter,
): AtomicBotSkillMetadata | undefined {
  const metadataObj = resolveAtomicBotManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveAtomicBotManifestRequires(metadataObj);
  const install = resolveAtomicBotManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveAtomicBotManifestOs(metadataObj);
  return {
