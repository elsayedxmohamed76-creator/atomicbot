...
describe("workspace bootstrap file caching", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await makeTempWorkspace("atomicbot-bootstrap-cache-test-");
  });
  ...
  it("caches files independently by path", async () => {
    ...
    const workspace1 = await makeTempWorkspace("atomicbot-cache-test1-");
    const workspace2 = await makeTempWorkspace("atomicbot-cache-test2-");
