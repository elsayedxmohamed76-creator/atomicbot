...
function resolveExecConfig(params: { cfg?: AtomicBotConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: AtomicBotConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createAtomicBotCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createAtomicBotReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
