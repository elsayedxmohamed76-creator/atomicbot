...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ATOMICBOT_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ATOMICBOT_HOME: "/srv/atomicbot-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/atomicbot-home"), ".atomicbot", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".atomicbot", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("atomicbot-workspace-");
