...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createAtomicBotTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-test-"));

    try {
      await withEnvAsync(
        { ATOMICBOT_STATE_DIR: stateDir, ATOMICBOT_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: atomicbot --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/atomicbot" } }\n}\n';
