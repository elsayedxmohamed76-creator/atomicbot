...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ATOMICBOT_HOME for tilde expansion", () => {
    vi.stubEnv("ATOMICBOT_HOME", "/srv/atomicbot-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/atomicbot-home", "cron", "jobs.json"));
  });
});
