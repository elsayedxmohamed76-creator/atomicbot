...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "atomicbot-cron-" });
}
...
export async function writeSessionStore(
  ...
  const dir = path.join(home, ".atomicbot", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AtomicBotConfig> = {},
): AtomicBotConfig {
  const base: AtomicBotConfig = {
    agents: {
      defaults: {
