...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: AtomicBotConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withStateDir = async (label: string, fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ ATOMICBOT_STATE_DIR: tmp }, async () => await fn(tmp));
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-security-audit-"));
  });
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.ATOMICBOT_GATEWAY_TOKEN;
    const prevPassword = process.env.ATOMICBOT_GATEWAY_PASSWORD;
    delete process.env.ATOMICBOT_GATEWAY_TOKEN;
    delete process.env.ATOMICBOT_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.ATOMICBOT_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.ATOMICBOT_GATEWAY_PASSWORD;
      } else {
        process.env.ATOMICBOT_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "atomicbot.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("atomicbot-sbx-browser-old\natomicbot-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "atomicbot-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "atomicbot-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("atomicbot-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("atomicbot-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "atomicbot-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "atomicbot-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-atomicbot.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): AtomicBotConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: AtomicBotConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withStateDir("discord-name-based-allowlist", async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.atomicbot/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: AtomicBotConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.ATOMICBOT_GATEWAY_TOKEN;
    process.env.ATOMICBOT_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: AtomicBotConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<AtomicBotConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: AtomicBotConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.atomicbot",
      configPath: "/Users/test/Dropbox/.atomicbot/atomicbot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: AtomicBotConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "atomicbot.json"),
      });
    ...
    const cfg: AtomicBotConfig = {
      plugins: {
        installs: {
          "voice-call": {
            source: "npm",
            spec: "@atomicbot/voice-call@1.2.3",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@atomicbot/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@atomicbot/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@atomicbot/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
  ...
  it("does not scan plugin code safety findings when deep audit is disabled", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        atomicbot: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        atomicbot: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    beforeEach(() => {
      envSnapshot = captureEnv(["ATOMICBOT_GATEWAY_TOKEN", "ATOMICBOT_GATEWAY_PASSWORD"]);
      delete process.env.ATOMICBOT_GATEWAY_TOKEN;
      delete process.env.ATOMICBOT_GATEWAY_PASSWORD;
    });
    ...
    const setProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        process.env.ATOMICBOT_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        process.env.ATOMICBOT_GATEWAY_PASSWORD = env.password;
      }
    };
