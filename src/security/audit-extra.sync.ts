...
function summarizeGroupPolicy(cfg: AtomicBotConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AtomicBotConfig): boolean {
...
function collectModels(cfg: AtomicBotConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: AtomicBotConfig): Set<string> {
  const baseCfg: AtomicBotConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AtomicBotConfig): boolean {
...
function isBrowserEnabled(cfg: AtomicBotConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AtomicBotConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep ATOMICBOT_STATE_DIR on a local-only volume and re-run "${formatCliCommand("atomicbot security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer ATOMICBOT_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const atomicbotGatewayToken =
    typeof env.ATOMICBOT_GATEWAY_TOKEN === "string" && env.ATOMICBOT_GATEWAY_TOKEN.trim()
      ? env.ATOMICBOT_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-atomicbot-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: atomicbot-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AtomicBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
