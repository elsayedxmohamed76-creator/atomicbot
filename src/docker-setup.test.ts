...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "atomicbot-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ATOMICBOT_GATEWAY_TOKEN: "test-token",
    ATOMICBOT_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ATOMICBOT_WORKSPACE_DIR: join(sandbox.rootDir, "atomicbot"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ATOMICBOT_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      ATOMICBOT_EXTRA_MOUNTS: undefined,
      ATOMICBOT_HOME_VOLUME: "atomicbot-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ATOMICBOT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("ATOMICBOT_EXTRA_MOUNTS=");
    expect(envFile).toContain("ATOMICBOT_HOME_VOLUME=atomicbot-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("atomicbot-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("atomicbot-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg ATOMICBOT_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ATOMICBOT_CONFIG_DIR: configDir,
      ATOMICBOT_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("rejects injected multiline ATOMICBOT_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ATOMICBOT_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ATOMICBOT_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ATOMICBOT_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ATOMICBOT_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ATOMICBOT_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ATOMICBOT_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ATOMICBOT_HOME_VOLUME must match");
  });
