...
import type { AtomicBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AtomicBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("atomicbot security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.atomicbot/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("atomicbot approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.atomicbot.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("atomicbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("atomicbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("atomicbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("atomicbot config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('atomicbot config set session.dmScope "per-channel-peer"') +
