...
export async function loadValidConfigOrThrow(): Promise<AtomicBotConfig> {
...
export async function updateConfig(
  mutator: (cfg: AtomicBotConfig) => AtomicBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AtomicBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AtomicBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("atomicbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
