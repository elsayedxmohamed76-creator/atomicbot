...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAtomicBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("atomicbot models auth setup-token")}\` or \`${formatCliCommand("atomicbot configure")}\`.`
          : `Run \`${formatCliCommand("atomicbot configure")}\` or set an API key env var.`;
