...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAtomicBotAgentDir: vi.fn().mockReturnValue("/tmp/atomicbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/atomicbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/atomicbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAtomicBotAgentDir: mocks.resolveAtomicBotAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/atomicbot-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAtomicBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/atomicbot-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/atomicbot-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/atomicbot-agent-custom");
