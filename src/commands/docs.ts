...
const SEARCH_TOOL = "https://docs.atomicbot.ai/mcp.SearchAtomicBot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.atomicbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('atomicbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.atomicbot.ai/");
      runtime.log(`Search: ${formatCliCommand('atomicbot docs "your query"')}`);
    }
    return;
  }
