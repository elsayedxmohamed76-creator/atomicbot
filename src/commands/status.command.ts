...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AtomicBot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("atomicbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("atomicbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.atomicbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.atomicbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("atomicbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("atomicbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("atomicbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("atomicbot gateway probe")}`);
  }
}
