...
beforeAll(() => {
  envSnapshot = captureEnv(["ATOMICBOT_PROFILE"]);
  process.env.ATOMICBOT_PROFILE = "isolated";
});
...
vi.mock("../infra/atomicbot-root.js", () => ({
  resolveAtomicBotPackageRoot: vi.fn().mockResolvedValue("/tmp/atomicbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/atomicbot/pnpm-lock.yaml",
      markerPath: "/tmp/atomicbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("AtomicBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("atomicbot status --all") ||
          l.includes("atomicbot --profile isolated status --all") ||
          l.includes("atomicbot status --all") ||
          l.includes("atomicbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ATOMICBOT_GATEWAY_TOKEN = "abcd1234";
    try {
