...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ATOMICBOT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("atomicbot-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-oauth-sync-"));
    ...
    process.env.ATOMICBOT_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-oauth-nosync-"));
    process.env.ATOMICBOT_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ATOMICBOT_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-oauth-external-"));
    process.env.ATOMICBOT_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ATOMICBOT_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ATOMICBOT_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("atomicbot-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
