...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ATOMICBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ATOMICBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ATOMICBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("ATOMICBOT_GATEWAY_PASSWORD");
  });
