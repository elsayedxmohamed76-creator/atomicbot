...
const ensureAtomicBotModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/atomicbot-agent/auth.json" });
const resolveAtomicBotAgentDir = vi.fn().mockReturnValue("/tmp/atomicbot-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/atomicbot-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureAtomicBotModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/atomicbot-agent");
  });
