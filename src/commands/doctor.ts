...
function resolveMode(cfg: AtomicBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AtomicBot doctor");

  const root = await resolveAtomicBotPackageRoot({
  ...
  let cfg: AtomicBotConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("atomicbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("atomicbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("atomicbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("atomicbot doctor --fix")}" to apply changes.`);
  }
