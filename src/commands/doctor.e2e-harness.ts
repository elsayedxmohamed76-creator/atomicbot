...
export const resolveAtomicBotPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadAtomicBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/atomicbot.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveAtomicBotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ATOMICBOT_STATE_DIR;
  originalUpdateInProgress = process.env.ATOMICBOT_UPDATE_IN_PROGRESS;
  process.env.ATOMICBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "atomicbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ATOMICBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ATOMICBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.ATOMICBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
