...
function resolveGatewayAuthToken(cfg: AtomicBotConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.ATOMICBOT_GATEWAY_TOKEN ?? env.ATOMICBOT_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (atomicbot/moltbot) now?",
      initialValue: true,
    });
    if (shouldRemove) {
      const removed: string[] = [];
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing AtomicBot gateway next.");
      }
    }
  }
