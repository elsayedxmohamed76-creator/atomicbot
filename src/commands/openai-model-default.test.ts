...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: AtomicBotConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: AtomicBotConfig) => { changed: boolean; next: AtomicBotConfig };
  defaultModel: string;
  overrideConfig: AtomicBotConfig;
  alreadyDefaultConfig: AtomicBotConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: AtomicBotConfig) => config,
      applyDefaultConfig: (config: AtomicBotConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: AtomicBotConfig = { agents: { defaults: {} } };
