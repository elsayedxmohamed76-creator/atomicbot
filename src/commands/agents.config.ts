...
function resolveAgentName(cfg: AtomicBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: AtomicBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AtomicBotConfig): AgentSummary[] {
