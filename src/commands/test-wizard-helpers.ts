...
export function requireAtomicBotAgentDir(): string {
  const agentDir = process.env.ATOMICBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ATOMICBOT_AGENT_DIR not set");
  }
  return agentDir;
}
