...
function stripUnknownConfigKeys(config: AtomicBotConfig): {
  config: AtomicBotConfig;
  removed: string[];
} {
  const parsed = AtomicBotSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: AtomicBotConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: AtomicBotConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: AtomicBotConfig): Promise<{
  config: AtomicBotConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: AtomicBotConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: AtomicBotConfig): {
  config: AtomicBotConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: AtomicBotConfig): {
  config: AtomicBotConfig;
  changes: string[];
} {
...
function collectExecSafeBinScopes(cfg: AtomicBotConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: AtomicBotConfig): ExecSafeBinCoverageHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: AtomicBotConfig): {
  config: AtomicBotConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: AtomicBotConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: AtomicBotConfig): {
  config: AtomicBotConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".atomicbot");
  const targetPath = path.join(targetDir, "atomicbot.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".atomicbot", "atomicbot.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: AtomicBotConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("atomicbot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("atomicbot doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("atomicbot doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("atomicbot doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("atomicbot doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("atomicbot doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("atomicbot doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "atomicbot doctor --fix" to remove these keys.');
    }
  }
