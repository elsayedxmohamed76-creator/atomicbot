...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AtomicBotConfig, policy: ChannelAccessPolicy) => AtomicBotConfig;
  resolveAllowlist: (params: { cfg: AtomicBotConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AtomicBotConfig; resolved: TResolved }) => AtomicBotConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AtomicBotConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AtomicBotConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AtomicBotConfig, policy: ChannelAccessPolicy): AtomicBotConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AtomicBotConfig, policy: ChannelAccessPolicy): AtomicBotConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AtomicBotConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AtomicBotConfig; resolved: string[] }) => {
      calls.push("apply");
