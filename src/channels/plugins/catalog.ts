...
import { discoverAtomicBotPlugins } from "../../plugins/discovery.js";
import type { AtomicBotPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, AtomicBotPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["ATOMICBOT_PLUGIN_CATALOG_PATHS", "ATOMICBOT_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<AtomicBotPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: AtomicBotPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: AtomicBotPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverAtomicBotPlugins({ workspaceDir: options.workspaceDir });
