...
function telegramCfg(): AtomicBotConfig {
  return { channels: { telegram: { botToken: "tok" } } } as AtomicBotConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: AtomicBotConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as AtomicBotConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as AtomicBotConfig;
...
function createSignalAccountOverrideCfg(): AtomicBotConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as AtomicBotConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as AtomicBotConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as AtomicBotConfig,
        accountId: undefined,
        params: {
