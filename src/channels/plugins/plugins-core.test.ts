...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@atomicbot/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "atomicbot-catalog-"));
        ...
        entries: [
          {
            ...
            atomicbot: {
              ...
              install: {
                npmSpec: "@atomicbot/demo-channel",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: AtomicBotConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
