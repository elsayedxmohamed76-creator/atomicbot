...
function cfg(accounts?: Record<string, unknown> | null): AtomicBotConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as AtomicBotConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as AtomicBotConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as AtomicBotConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as AtomicBotConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as AtomicBotConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as AtomicBotConfig)).toBe("default");
    });
  });
});
