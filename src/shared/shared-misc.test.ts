...
describe("shared/frontmatter", () => {
  ...
  test("resolveAtomicBotManifestBlock parses JSON5 metadata and picks atomicbot block", () => {
    const frontmatter = {
      metadata: "{ atomicbot: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveAtomicBotManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveAtomicBotManifestBlock returns undefined for invalid input", () => {
    expect(resolveAtomicBotManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveAtomicBotManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveAtomicBotManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
