# AtomicBot ACP Bridge

This document describes how the AtomicBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`atomicbot acp` exposes an ACP agent over stdio and forwards prompts to a running
AtomicBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `atomicbot acp` and speaks ACP messages over stdio.
...
`atomicbot acp` resolves the Gateway URL and auth from CLI flags or config:
