...
IMAGE_NAME="atomicbot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="atomicbot-net-e2e-$$"
GW_NAME="atomicbot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ATOMICBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "ATOMICBOT_SKIP_CHANNELS=1" \
	  -e "ATOMICBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "ATOMICBOT_SKIP_CRON=1" \
	  -e "ATOMICBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
