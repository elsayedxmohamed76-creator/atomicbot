...
show_usage() {
    cat << EOF
clawlog - AtomicBot Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View AtomicBot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    AtomicBot logs flow through the macOS unified log (subsystem: ai.atomicbot).
