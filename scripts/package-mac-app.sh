...
# Build and bundle AtomicBot into a minimal .app we can open.
# Outputs to dist/AtomicBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AtomicBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AtomicBot"
BUNDLE_ID="${BUNDLE_ID:-ai.atomicbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/atomicbot/atomicbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AtomicBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AtomicBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AtomicBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AtomicBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AtomicBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AtomicBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AtomicBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AtomicBot/Resources/AtomicBot.icns" "$APP_ROOT/Contents/Resources/AtomicBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AtomicBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AtomicBotKit resources"
ATOMICBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AtomicBotKit_AtomicBotKit.bundle"
if [ -d "$ATOMICBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AtomicBotKit_AtomicBotKit.bundle"
  cp -R "$ATOMICBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AtomicBotKit_AtomicBotKit.bundle"
else
  echo "WARN: AtomicBotKit resource bundle not found at $ATOMICBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AtomicBot"
killall -q AtomicBot 2>/dev/null || true
