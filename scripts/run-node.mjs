...
const shouldBuild = (deps) => {
  if (deps.env.ATOMICBOT_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ATOMICBOT_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[atomicbot] ${message}\n`);
};

const runAtomicBot = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["atomicbot.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runAtomicBot(deps);
  }
