...
ZIP=${1:?"Usage: $0 AtomicBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/atomicbot/atomicbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^AtomicBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/atomicbot/atomicbot/releases/download/v${VERSION}/"}
