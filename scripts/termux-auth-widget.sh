...
# AtomicBot Auth Widget for Termux
...
# Check auth status
termux-toast "Checking AtomicBot auth..."

STATUS=$(ssh "$SERVER" '$HOME/atomicbot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/atomicbot/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ATOMICBOT_EXPIRING|ATOMICBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/atomicbot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|ATOMICBOT_EXPIRED|ATOMICBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AtomicBot won't work until re-auth"
                ;;
        esac
        ;;
