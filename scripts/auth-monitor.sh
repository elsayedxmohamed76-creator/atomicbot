...
#
# Suggested cron: */30 * * * * /home/admin/atomicbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AtomicBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., atomicbot-alerts)
...
STATE_FILE="$HOME/.atomicbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AtomicBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use atomicbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AtomicBot to $NOTIFY_PHONE..."
            atomicbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AtomicBot is down. Run: ssh l36 '~/atomicbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
