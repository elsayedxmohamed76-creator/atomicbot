...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/atomicbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AtomicBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/atomicbot/scripts/mobile-reauth.sh'" --id atomicbot-auth
        ;;
    *)
