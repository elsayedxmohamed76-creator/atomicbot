...
INSTALL_URL="${ATOMICBOT_INSTALL_URL:-https://atomicbot.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ATOMICBOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ATOMICBOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="atomicbot"
PACKAGE_NAME="${ATOMICBOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ATOMICBOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ATOMICBOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ATOMICBOT_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
