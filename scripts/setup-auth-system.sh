#!/bin/bash
# Setup AtomicBot Auth Management System
...
echo "=== AtomicBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'atomicbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AtomicBot message: Send warning via AtomicBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/atomicbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/atomicbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/atomicbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now atomicbot-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/AtomicAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/AtomicAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Atomic*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (AtomicAuth) shows status and opens auth URL if needed."
echo "The full widget (AtomicAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status atomicbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
