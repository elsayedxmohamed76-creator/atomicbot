...
import type { AtomicBotConfig } from "../src/config/config.js";
import { resolvePreferredAtomicBotTmpDir } from "../src/infra/tmp-atomicbot-dir.js";
...
const makeTempDir = async (prefix: string) => {
  const baseDir = resolvePreferredAtomicBotTmpDir();
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("atomicbot-media-e2e-");
...
const envSnapshot = () => ({
  ...
  ATOMICBOT_AGENT_DIR: process.env.ATOMICBOT_AGENT_DIR,
...
const restoreEnv = (snapshot: ReturnType<typeof envSnapshot>) => {
  ...
  restoreEnvVar("ATOMICBOT_AGENT_DIR", snapshot.ATOMICBOT_AGENT_DIR);
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it.skipIf(process.platform === "win32")("uses sherpa-onnx-offline when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "atomicbot-bin-sherpa-");
      const modelDir = await createTrackedTempDir(tempPaths, "atomicbot-sherpa-model-");
      ...
      const cfg: AtomicBotConfig = { tools: { media: { audio: {} } } };
  ...
  it.skipIf(process.platform === "win32")("uses whisper-cli when sherpa is missing", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "atomicbot-bin-whispercpp-");
      const modelDir = await createTrackedTempDir(tempPaths, "atomicbot-whispercpp-model-");
  ...
  it.skipIf(process.platform === "win32")("uses gemini CLI for images when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "atomicbot-bin-gemini-");
      ...
      const cfg: AtomicBotConfig = { tools: { media: { image: {} } } };
  ...
  it("skips auto-detect when no supported binaries are available", async () => {
    await withEnvSnapshot(async () => {
      const emptyBinDir = await createTrackedTempDir(tempPaths, "atomicbot-bin-empty-");
      const isolatedAgentDir = await createTrackedTempDir(tempPaths, "atomicbot-agent-empty-");
      ...
      process.env.ATOMICBOT_AGENT_DIR = isolatedAgentDir;
