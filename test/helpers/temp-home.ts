...
type EnvSnapshot = {
  ...
  atomicbotHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    atomicbotHome: process.env.ATOMICBOT_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ATOMICBOT_HOME", snapshot.atomicbotHome);
  restoreKey("ATOMICBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ATOMICBOT_HOME.
  delete process.env.ATOMICBOT_HOME;
  process.env.ATOMICBOT_STATE_DIR = path.join(base, ".atomicbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "atomicbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".atomicbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
