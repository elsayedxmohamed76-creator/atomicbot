...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `atomicbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".atomicbot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ATOMICBOT_SKIP_PROVIDERS: "1",
          ATOMICBOT_SKIP_GMAIL_WATCHER: "1",
          ATOMICBOT_SKIP_CRON: "1",
          ATOMICBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
          ATOMICBOT_SKIP_CANVAS_HOST: "1",
          ATOMICBOT_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
