...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/atomicbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/atomicbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/atomicbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ATOMICBOT_CONTROL_UI_BASE_PATH__ = "/atomicbot";
    const app = mountApp("/atomicbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/atomicbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/atomicbot/sessions");
  });
