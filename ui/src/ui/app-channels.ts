...
export async function handleWhatsAppStart(host: AtomicBotApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AtomicBotApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AtomicBotApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AtomicBotApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AtomicBotApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AtomicBotApp): string {
...
function resolveGatewayHttpAuthHeader(host: AtomicBotApp): string | null {
...
function buildGatewayHttpHeaders(host: AtomicBotApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AtomicBotApp) {
...
export function handleNostrProfileToggleAdvanced(host: AtomicBotApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AtomicBotApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AtomicBotApp) {
  const state = host.nostrProfileFormState;
