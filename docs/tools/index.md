---
summary: "Agent tool surface for AtomicBot (browser, canvas, nodes, message, cron) replacing legacy `atomicbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `atomicbot-*` skills
title: "Tools"
---

# Tools (AtomicBot)

AtomicBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `atomicbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `atomicbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AtomicBot logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:atomicbot`: all built-in AtomicBot tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`atomicbot node run`).
...
AtomicBot tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `atomicbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated AtomicBot-managed browser.
...
- Quick smoke: `atomicbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `atomicbot gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, AtomicBot clamps `tools.sessions.visibility` to `tree`.
