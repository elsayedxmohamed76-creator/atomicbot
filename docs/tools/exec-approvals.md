...
- **gateway host** → `atomicbot` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.atomicbot/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.atomicbot/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `atomicbot approvals allowlist ...`).
- `atomicbot security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `atomicbot doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.atomicbot/exec-approvals.json` directly.

CLI: `atomicbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
