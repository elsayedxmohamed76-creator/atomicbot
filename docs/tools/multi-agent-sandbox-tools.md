...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `atomicbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `atomicbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=atomicbot-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ATOMICBOT_STATE_DIR:-$HOME/.atomicbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
