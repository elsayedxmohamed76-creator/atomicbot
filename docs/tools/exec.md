...
- `gateway`/`node` approvals are controlled by `~/.atomicbot/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  AtomicBot prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`atomicbot security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `atomicbot doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
