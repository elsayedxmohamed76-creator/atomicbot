---
summary: "Chrome extension: let AtomicBot drive your existing Chrome tab"
read_when:
...
The AtomicBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate atomicbot-managed Chrome profile.
...
AtomicBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AtomicBot release (npm package) as static files. There is no separate “build” step.

After upgrading AtomicBot:

- Re-run `atomicbot browser extension install` to refresh the installed files under your AtomicBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

AtomicBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ATOMICBOT_GATEWAY_TOKEN`)
...
- Open the tab you want AtomicBot to control.
...
- `ON`: attached; AtomicBot can drive that tab.
...
Debugging: `atomicbot sandbox explain`
...
`atomicbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `atomicbot browser extension install` first to copy the extension to a stable location under your AtomicBot state directory.
...
- **This is not isolated** like the dedicated atomicbot-managed profile.
