...
- For manual spawns, delivery is resilient:
  - AtomicBot tries direct `agent` delivery first with a stable idempotency key.
...
2. AtomicBot creates or binds a thread to that session target in the active channel.
