---
summary: "Synology Chat webhook setup and AtomicBot config"
read_when:
  - Setting up Synology Chat with AtomicBot
...
```bash
atomicbot plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your AtomicBot gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in AtomicBot.
...
- Pairing approvals work with:
  - `atomicbot pairing list synology-chat`
  - `atomicbot pairing approve synology-chat <CODE>`
...
```bash
atomicbot message send --channel synology-chat --target 123456 --text "Hello from AtomicBot"
atomicbot message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
