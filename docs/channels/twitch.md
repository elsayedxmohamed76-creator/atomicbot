...
read_when:
  - Setting up Twitch chat integration for AtomicBot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AtomicBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
atomicbot plugins install @atomicbot/twitch
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ATOMICBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "atomicbot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ATOMICBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
ATOMICBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
