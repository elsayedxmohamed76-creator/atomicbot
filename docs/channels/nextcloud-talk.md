...
```bash
atomicbot plugins install @atomicbot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AtomicBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `atomicbot pairing list nextcloud-talk`
  - `atomicbot pairing approve nextcloud-talk <CODE>`
