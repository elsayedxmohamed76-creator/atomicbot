---
summary: "Mattermost bot setup and AtomicBot config"
read_when:
...
```bash
atomicbot plugins install @atomicbot/mattermost
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/mattermost
```
...
4. Configure AtomicBot and start the gateway.
...
- Approve via:
  - `atomicbot pairing list mattermost`
  - `atomicbot pairing approve mattermost <CODE>`
...
Use these target formats with `atomicbot message send` or cron/webhooks:
