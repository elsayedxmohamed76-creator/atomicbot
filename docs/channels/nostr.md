...
read_when:
  - You want AtomicBot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AtomicBot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`atomicbot onboard`) and `atomicbot channels add` list optional channel plugins.
...
```bash
atomicbot plugins install @atomicbot/nostr
```

Use a local checkout (dev workflows):

```bash
atomicbot plugins install --link <path-to-atomicbot>/extensions/nostr
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "atomicbot",
        "displayName": "AtomicBot",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "atomicbot@example.com",
        "lud16": "atomicbot@example.com"
      }
    }
  }
}
