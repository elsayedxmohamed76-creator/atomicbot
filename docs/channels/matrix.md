...
Matrix is an open, decentralized messaging protocol. AtomicBot connects as a Matrix **user**
...
```bash
atomicbot plugins install @atomicbot/matrix
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `atomicbot plugins install @atomicbot/matrix`
   - From a local checkout: `atomicbot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AtomicBot calls the same
     login endpoint, stores the access token in `~/.atomicbot/credentials/matrix/credentials.json`,
...
- On first connection, AtomicBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AtomicBot logs a warning.
...
`~/.atomicbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `atomicbot pairing list matrix`
  - `atomicbot pairing approve matrix <CODE>`
...
- On startup, AtomicBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
