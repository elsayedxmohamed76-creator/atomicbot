...
Tlon is a decentralized messenger built on Urbit. AtomicBot connects to your Urbit ship and can
...
```bash
atomicbot plugins install @atomicbot/tlon
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/tlon
```
...
By default, AtomicBot blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `atomicbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AtomicBot replies in-thread.
