...
```bash
atomicbot plugins install @atomicbot/msteams
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
AtomicBot will offer the local install path automatically.
...
3. Configure AtomicBot with those credentials.
...
- Talk to AtomicBot via Teams DMs, group chats, or channels.
...
- On startup, AtomicBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.atomicbot/atomicbot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring AtomicBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `atomicbot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `atomicbot plugins install @atomicbot/msteams`
   - From a local checkout: `atomicbot plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "AtomicBot in Teams", "full": "AtomicBot in Teams" },
...
AtomicBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, AtomicBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AtomicBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

AtomicBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `atomicbot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.atomicbot/msteams-polls.json`.
...
```bash
atomicbot message send --channel msteams \
...
atomicbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
atomicbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
atomicbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
atomicbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
