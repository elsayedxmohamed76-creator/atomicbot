...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AtomicBot to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
atomicbot plugins install @atomicbot/feishu
```

Local checkout (when running from a git repo):

```bash
atomicbot plugins install ./extensions/feishu
```
...
If you just installed AtomicBot, run the wizard:
...
2. Configuring app credentials in AtomicBot
...
1. You already ran `atomicbot channels add` for Feishu
2. The gateway is running (`atomicbot gateway status`)
...
```bash
atomicbot pairing approve feishu <CODE>
```
...
2. Run `atomicbot logs --follow` and look for `chat_id`
...
2. Run `atomicbot logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `atomicbot gateway status`  | Show gateway status           |
| `atomicbot gateway install` | Install/start gateway service |
| `atomicbot gateway stop`    | Stop gateway service          |
| `atomicbot gateway restart` | Restart gateway service       |
| `atomicbot logs --follow`   | Tail gateway logs             |
...
4. Check logs: `atomicbot logs --follow`
...
5. Ensure the gateway is running: `atomicbot gateway status`
6. Check logs: `atomicbot logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "atomic-fan",
        workspace: "/home/user/atomic-fan",
        agentDir: "/home/user/.atomicbot/agents/atomic-fan/agent",
      },
      {
        id: "atomic-xi",
        workspace: "/home/user/atomic-xi",
        agentDir: "/home/user/.atomicbot/agents/atomic-xi/agent",
      },
    ],
  },
  bindings: [
