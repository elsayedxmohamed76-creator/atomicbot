---
summary: "CLI reference for `atomicbot security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `atomicbot.browserConfigEpoch`) and recommends `atomicbot sandbox recreate --browser --all`.
...
```bash
atomicbot security audit --json | jq '.summary'
atomicbot security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
atomicbot security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
