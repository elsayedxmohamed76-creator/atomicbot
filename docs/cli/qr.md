---
summary: "CLI reference for `atomicbot qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
atomicbot qr
atomicbot qr --setup-code-only
atomicbot qr --json
atomicbot qr --remote
atomicbot qr --url wss://gateway.example/ws --token '<token>'
```
