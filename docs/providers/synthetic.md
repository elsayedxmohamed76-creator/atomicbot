---
summary: "Use Synthetic's Anthropic-compatible API in AtomicBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. AtomicBot registers it as the
...
Note: AtomicBot's Anthropic client appends `/v1` to the base URL, so use
