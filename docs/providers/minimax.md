---
summary: "Use MiniMax M2.1 in AtomicBot"
read_when:
  - You want MiniMax models in AtomicBot
...
```bash
atomicbot plugins enable minimax-portal-auth  # skip if already loaded.
atomicbot gateway restart  # restart if gateway is already running
atomicbot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/atomicbot/atomicbot/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `atomicbot.json`:
...
- Use `atomicbot models list` and `atomicbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `atomicbot configure` and selecting **MiniMax M2.1**, or
