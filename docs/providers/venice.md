---
summary: "Use Venice AI privacy-focused models in AtomicBot"
read_when:
  - You want privacy-focused inference in AtomicBot
...
```bash
atomicbot onboard --auth-choice venice-api-key
```
...
```bash
atomicbot agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, AtomicBot shows all available Venice models. Pick based on your needs:
...
```bash
atomicbot models set venice/claude-opus-45
atomicbot models set venice/llama-3.3-70b
```

List all available models:

```bash
atomicbot models list | grep venice
```

## Configure via `atomicbot configure`

1. Run `atomicbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AtomicBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
atomicbot agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
atomicbot agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
atomicbot agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
atomicbot agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
atomicbot agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `atomicbot models list` to see currently available models. Some models may be temporarily offline.
