...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AtomicBot
...
Running AtomicBot via Node with `tsx` fails at startup with:

```
[atomicbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `atomicbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch atomicbot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node atomicbot.mjs status` works on Node 25.
