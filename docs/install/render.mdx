...
Deploy AtomicBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/atomicbot/atomicbot)
...
repository configures everything needed to run AtomicBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ATOMICBOT_STATE_DIR
        value: /data/.atomicbot
      - key: ATOMICBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: ATOMICBOT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/atomicbot`.
...
If you use the original AtomicBot repository, Render will not auto-deploy your AtomicBot. To update it, run a manual Blueprint sync from the dashboard.
...
For AtomicBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AtomicBot host.
