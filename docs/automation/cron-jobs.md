...
- Jobs persist under `~/.atomicbot/cron/` so restarts don’t lose schedules.
...
atomicbot cron list
atomicbot cron run <job-id>
atomicbot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.atomicbot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `atomicbot cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, AtomicBot applies a
...
If `delivery` is omitted for isolated jobs, AtomicBot defaults to `announce`.
...
- Job store: `~/.atomicbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.atomicbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.atomicbot/cron/jobs.json",
...
atomicbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
atomicbot cron edit <jobId> --agent ops
atomicbot cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
atomicbot cron run <jobId>
atomicbot cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
atomicbot cron edit <jobId> \
...
```bash
atomicbot cron edit <jobId> --exact
```
...
```bash
atomicbot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`atomicbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ATOMICBOT_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- AtomicBot applies exponential retry backoff for recurring jobs after consecutive errors:
