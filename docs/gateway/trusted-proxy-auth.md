...
read_when:
  - Running AtomicBot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of AtomicBot
...
- You run AtomicBot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. AtomicBot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. AtomicBot extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://atomicbot.example.com
    to: http://atomicbot-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
atomicbot.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy atomicbot:18789 {
...
location / {
    ...
    proxy_pass http://atomicbot:18789;
...
`atomicbot security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update AtomicBot config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `atomicbot security audit` and review findings
