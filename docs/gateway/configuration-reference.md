...
description: "Complete field-by-field reference for ~/.atomicbot/atomicbot.json"
summary: "Complete reference for every AtomicBot config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.atomicbot/atomicbot.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — AtomicBot uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `atomicbot doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["atomicbot-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `atomicbot plugins install @atomicbot/mattermost`.
...
AtomicBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@atomicbot"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `atomicbot.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, AtomicBot auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, AtomicBot assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "atomicbot-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.atomicbot/sandboxes`
...
noVNC observer access uses VNC auth by default and AtomicBot emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `atomicbot-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.atomicbot/workspace",
        agentDir: "~/.atomicbot/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@atomicbot"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:atomicbot`   | All built-in tools (excludes provider plugins)                                           |
...
AtomicBot uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.atomicbot/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ATOMICBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `atomicbot onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `atomicbot onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `atomicbot onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `atomicbot onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `atomicbot onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `atomicbot onboard --auth-choice minimax-api`.
...
- Loaded from `~/.atomicbot/extensions`, `<workspace>/.atomicbot/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ATOMICBOT_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ATOMICBOT_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.atomicbot-dev` + port `19001`), `--profile <name>` (uses `~/.atomicbot-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.atomicbot/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-atomicbot-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ATOMICBOT_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ATOMICBOT_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__atomicbot__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__atomicbot__/a2ui/`
...
- Also serves A2UI at `/__atomicbot__/a2ui/`.
...
- Hostname defaults to `atomicbot`. Override with `ATOMICBOT_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.atomicbot/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `atomicbot dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.atomicbot/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ATOMICBOT_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.atomicbot/credentials/oauth.json`.
...
- Default log file: `/tmp/atomicbot/atomicbot-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `atomicbot doctor --fix` can strip unknown keys).
