...
- During heartbeat runs, AtomicBot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `atomicbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AtomicBot skips the heartbeat run entirely (no model call).
...
```bash
atomicbot system event --text "Check for urgent follow-ups" --mode now
```
