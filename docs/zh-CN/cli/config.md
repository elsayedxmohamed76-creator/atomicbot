...
read_when:
  - 你想以非交互方式读取或编辑配置
summary: "`atomicbot config` 的 CLI 参考（获取/设置/取消设置配置值）"
title: config
x-i18n:
...
atomicbot config set gateway.port 19001 --json
atomicbot config set channels.whatsapp.groups '["*"]' --json
```
