...
你可以在 `~/.atomicbot/atomicbot.json` 中覆盖此设置：
...
所有日志配置都在 `~/.atomicbot/atomicbot.json` 的 `logging` 下。
...
- AtomicBot 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `atomicbot plugins enable diagnostics-otel` 启用插件。
...
- `atomicbot.tokens`（计数器，属性：`atomicbot.token`、`atomicbot.channel`、`atomicbot.provider`、`atomicbot.model`）
- `atomicbot.cost.usd`（计数器，属性：`atomicbot.channel`、`atomicbot.provider`、`atomicbot.model`）
- `atomicbot.run.duration_ms`（直方图，属性：`atomicbot.channel`、`atomicbot.provider`、`atomicbot.model`）
- `atomicbot.context.tokens`（直方图，属性：`atomicbot.context`、`atomicbot.channel`、`atomicbot.provider`、`atomicbot.model`）

消息流：

- `atomicbot.webhook.received`（计数器，属性：`atomicbot.channel`、`atomicbot.webhook`）
- `atomicbot.webhook.error`（计数器，属性：`atomicbot.channel`、`atomicbot.webhook`）
- `atomicbot.webhook.duration_ms`（直方图，属性：`atomicbot.channel`、`atomicbot.webhook`）
- `atomicbot.message.queued`（计数器，属性：`atomicbot.channel`、`atomicbot.source`）
- `atomicbot.message.processed`（计数器，属性：`atomicbot.channel`、`atomicbot.outcome`）
- `atomicbot.message.duration_ms`（直方图，属性：`atomicbot.channel`、`atomicbot.outcome`）

队列 + 会话：

- `atomicbot.queue.lane.enqueue`（计数器，属性：`atomicbot.lane`）
- `atomicbot.queue.lane.dequeue`（计数器，属性：`atomicbot.lane`）
- `atomicbot.queue.depth`（直方图，属性：`atomicbot.lane` 或 `atomicbot.channel=heartbeat`）
- `atomicbot.queue.wait_ms`（直方图，属性：`atomicbot.lane`）
- `atomicbot.session.state`（计数器，属性：`atomicbot.state`、`atomicbot.reason`）
- `atomicbot.session.stuck`（计数器，属性：`atomicbot.state`）
- `atomicbot.session.stuck_age_ms`（直方图，属性：`atomicbot.state`）
- `atomicbot.run.attempt`（计数器，属性：`atomicbot.attempt`）
...
- `atomicbot.webhook.error`
  - `atomicbot.channel`、`atomicbot.webhook`、`atomicbot.chatId`、`atomicbot.error`
- `atomicbot.message.processed`
  - `atomicbot.channel`、`atomicbot.outcome`、`atomicbot.chatId`、`atomicbot.messageId`、`atomicbot.sessionKey`、`atomicbot.sessionId`、`atomicbot.reason`
- `atomicbot.session.stuck`
  - `atomicbot.state`、`atomicbot.ageMs`、`atomicbot.queueDepth`、`atomicbot.sessionKey`、`atomicbot.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `atomicbot doctor`。
