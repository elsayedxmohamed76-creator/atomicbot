---
summary: "AtomicBot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AtomicBot on a Raspberry Pi
  - Running AtomicBot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# AtomicBot on Raspberry Pi

## Goal

Run a persistent, always-on AtomicBot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most AtomicBot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled atomicbot

# Enable if not
sudo systemctl enable atomicbot

# Start on boot
sudo systemctl start atomicbot
```
...
```bash
# Check logs
journalctl -u atomicbot --no-pager -n 100

# Common fix: rebuild
cd ~/atomicbot  # if using hackable install
npm run build
sudo systemctl restart atomicbot
```
