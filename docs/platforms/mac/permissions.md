...
- Same path: run the app from a fixed location (for AtomicBot, `dist/AtomicBot.app`).
...
Workaround: move files into the AtomicBot workspace (`~/.atomicbot/workspace`) if you want to avoid per-folder grants.
