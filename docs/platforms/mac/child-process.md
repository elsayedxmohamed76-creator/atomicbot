...
service via the external `atomicbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ATOMICBOT_PROFILE`; legacy `com.atomicbot.*` is supported).
