...
```bash
atomicbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `atomicbot tui --deliver`
...
- Check the Gateway logs: `atomicbot logs --follow`.
- Confirm the agent can run: `atomicbot status` and `atomicbot models status`.
...
- No agents in picker: check `atomicbot agents list` and your routing config.
