...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/atomicbot`)
...
If the page fails to load, start the Gateway first: `atomicbot gateway`.
...
you revoke it with `atomicbot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.atomicbot/atomicbot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AtomicBot
...
AtomicBot **blocks** Control UI connections without device identity.
