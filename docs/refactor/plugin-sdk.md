...
- Publish as `atomicbot/plugin-sdk` (or export from core under `atomicbot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AtomicBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AtomicBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AtomicBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AtomicBotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AtomicBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AtomicBotConfig): string;
  };
};
```
...
- Introduce `atomicbot/plugin-sdk`.
- Add `api.runtime` to `AtomicBotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `atomicbotRuntime: ">=2026.2.0"`).
