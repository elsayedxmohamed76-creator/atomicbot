...
AtomicBot features that can generate provider usage or paid API calls.
...
- `atomicbot status --usage` and `atomicbot channels list` show provider **usage windows**
...
AtomicBot can pick up credentials from:
...
`atomicbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
