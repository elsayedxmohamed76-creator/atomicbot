---
summary: "How AtomicBot builds prompt context and reports token usage + costs"
read_when:
...
AtomicBot tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

AtomicBot assembles its own system prompt on every run. It includes:
...
For images, AtomicBot downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from AtomicBot session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `atomicbot status --usage` and `atomicbot channels list` show
...
`cacheWrite`. If pricing is missing, AtomicBot shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. AtomicBot can
...
Anthropic's 1M context window is currently beta-gated. AtomicBot can inject the
