...
AtomicBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AtomicBot owns lifecycle when `channels.signal.autoStart=true`.
...
- AtomicBot spawns `imsg rpc` as a child process (legacy iMessage integration).
