...
- `/usage cost` in chats: local cost summary aggregated from AtomicBot session logs.
- CLI: `atomicbot status --usage` prints a full per-provider breakdown.
- CLI: `atomicbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
