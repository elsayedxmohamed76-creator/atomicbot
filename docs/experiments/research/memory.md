---
summary: "Research notes: offline memory system for Atomic workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.atomicbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AtomicBot integration
...
Target: Atomic-style workspace (`agents.defaults.workspace`, default `~/.atomicbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.atomicbot/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Atomic to see every session.
...
```
~/.atomicbot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AtomicBot**, but keep a separable core library.

### Why integrate into AtomicBot?

- AtomicBot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `atomicbot memory recall "…" --k 25 --since 30d`
  - `atomicbot memory reflect --since 7d`
...
Pragmatic take for `~/.atomicbot/workspace`:
