...
AtomicBot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
atomicbot channels login --channel zalouser
...
```bash
atomicbot message send --channel zalouser --target <threadId> --message "Hello from AtomicBot!"
```
...
```bash
atomicbot channels login --channel zalouser              # Login via QR
atomicbot channels login --channel zalouser --account work
atomicbot channels status --probe
atomicbot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
atomicbot directory self --channel zalouser
atomicbot directory peers list --channel zalouser --query "name"
atomicbot directory groups list --channel zalouser --query "work"
atomicbot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
atomicbot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
atomicbot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `atomicbot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
atomicbot channels login --channel zalouser --account work
atomicbot message send --channel zalouser --account work --target <id> --message "Hello"
