...
import type { PluginRuntime } from "atomicbot/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ATOMICBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "atomicbot-nostr-"));
  process.env.ATOMICBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.ATOMICBOT_STATE_DIR?.trim() || stateEnv.ATOMICBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".atomicbot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ATOMICBOT_STATE_DIR = previous;
    }
