...
metadata: { "atomicbot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AtomicBot `sessions_spawn`
- **File I/O** == AtomicBot `read`/`write`
- **Remote fetch** == AtomicBot `web_fetch` (or `exec` with curl when POST is required)
