...
import { createDedupeCache, createPersistentDedupe } from "atomicbot/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.ATOMICBOT_STATE_DIR?.trim() || env.ATOMICBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["atomicbot-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".atomicbot");
}
