...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["atomicbotPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["atomicbot", "pollId"]) ??
    readNestedString(value, ["atomicbot", "poll", "id"]) ??
    readNestedString(value, ["data", "atomicbotPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "atomicbot", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          atomicbotPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "atomicbot poll vote",
          displayText: "Vote recorded",
          value: { atomicbotPollId: pollId, pollId },
        },
      },
    ],
  };
