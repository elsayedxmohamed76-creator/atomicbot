...
`atomicbot.invoke --each`) without adding new AtomicBot code per workflow.
...
This extension depends on AtomicBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AtomicBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.atomicbot/extensions` as a standalone plugin directory.
