...
import { approveDevicePairing, listDevicePairing } from "atomicbot/plugin-sdk";
...
function resolveGatewayPort(cfg: AtomicBotPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ATOMICBOT_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.ATOMICBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: AtomicBotPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: AtomicBotPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the AtomicBot iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the AtomicBot iOS app:",
            "",
            "```",
            qrAscii,
            "```",
