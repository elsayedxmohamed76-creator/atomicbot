...
import type { AtomicBotPluginApi, AtomicBotPluginService } from "atomicbot/plugin-sdk";
...
function normalizeDenyList(cfg: AtomicBotPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AtomicBotPluginApi["config"]): string[] {
...
export default function register(api: AtomicBotPluginApi) {
  ...
  const timerService: AtomicBotPluginService = {
