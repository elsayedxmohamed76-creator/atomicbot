import {
  ...
  type AtomicBotConfig,
  type RuntimeEnv,
} from "atomicbot/plugin-sdk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as AtomicBotConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AtomicBotConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AtomicBotConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AtomicBotConfig);
