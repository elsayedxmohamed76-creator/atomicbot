...
function resolveAtomicBotRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ATOMICBOT_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ATOMICBOT_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAtomicBotRoot(), "dist", "extensionAPI.js");
