...
import type { ReplyPayload, AtomicBotConfig } from "atomicbot/plugin-sdk";
import { createReplyPrefixOptions } from "atomicbot/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // AtomicBotConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as AtomicBotConfig;
