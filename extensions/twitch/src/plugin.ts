/**
 * Twitch channel plugin for AtomicBot.
 *
...
import { buildChannelConfigSchema } from "atomicbot/plugin-sdk";
 ...
 * for AtomicBot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AtomicBotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AtomicBotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AtomicBotConfig): boolean => {
