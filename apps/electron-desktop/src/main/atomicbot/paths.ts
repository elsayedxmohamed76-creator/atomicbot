...
export function resolveRepoRoot(mainDir: string): string {
  ...
  // We want the repo root to locate atomicbot.mjs and dist/.
  return path.resolve(mainDir, "..", "..", "..");
}

export function resolveBundledAtomicBotDir(): string {
  return path.join(process.resourcesPath, "atomicbot");
}
