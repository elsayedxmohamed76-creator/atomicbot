...
export function registerGogIpcHandlers(params: {
  ...
  const { gogBin, atomicbotDir, userData, stateDir } = params;
  ...
  ipcMain.handle("gog-auth-list", async () => {
    ...
    const res = await runGog({ bin: gogBin, args: ["auth", "list"], cwd: atomicbotDir });
    return res;
  });
  ...
  ipcMain.handle(
    ...
    async (_evt, p: { account?: unknown; services?: unknown; noInput?: unknown }) => {
      ...
      const res = await runGog({ bin: gogBin, args, cwd: atomicbotDir });
      return res;
    }
  );
