...
/**
 * Ensure a helper bin directory exists inside stateDir with an `atomicbot`
 * wrapper script that points to the real atomicbot.mjs entry point.
 * This lets the user type `atomicbot <cmd>` in the embedded terminal.
 */
function ensureTerminalBinDir(params: {
  ...
  const atomicbotMjs = path.join(params.atomicbotDir, "atomicbot.mjs");
  getPlatform().createCliWrapper({
    binDir,
    name: "atomicbot",
    nodeBin: params.nodeBin,
    scriptPath: atomicbotMjs,
  });
 ...
 * `atomicbot`, `node`, `gog`, `jq`, etc. directly from the embedded terminal.
 */
function buildTerminalPath(params: {
  ...
  // 1. The helper bin dir with the `atomicbot` wrapper script (highest priority).
...
export function createTerminal(params: CreateTerminalParams): { id: string } {
  ...
  // Create/refresh the helper bin directory with the `atomicbot` wrapper script.
  const terminalBinDir = ensureTerminalBinDir({
    stateDir: params.stateDir,
    atomicbotDir: params.atomicbotDir,
    nodeBin: params.nodeBin,
  });
