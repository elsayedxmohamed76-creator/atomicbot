...
export function registerResetAndCloseIpcHandler(params: {
  ...
  const { userData, stateDir, logsDir, whisperDataDir, gogBin, atomicbotDir, stopGatewayChild } =
    params;
  ...
  ipcMain.handle("reset-and-close", async () => {
    ...
    try {
      await clearGogAuthTokens({ gogBin, atomicbotDir, warnings });
    } catch (err) {
      warnings.push(`failed to clear gog auth tokens: ${String(err)}`);
    }

    // Clear the embedded AtomicBot state/logs, downloaded whisper models/ffmpeg,
