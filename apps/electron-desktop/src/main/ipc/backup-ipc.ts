 ...
 * directory with `atomicbot.json` inside, use that; otherwise use extractDir itself.
 */
async function resolveBackupRoot(extractDir: string): Promise<string> {
  // Check extractDir directly first
  try {
    await fsp.stat(path.join(extractDir, "atomicbot.json"));
    return extractDir;
  } catch {
    // not at top level
  }
  ...
  if (dirs.length === 1 && dirs[0]) {
    ...
    try {
      await fsp.stat(path.join(candidate, "atomicbot.json"));
      return candidate;
    } catch {
      // not there either
    }
  ...
  throw new Error("Invalid backup: atomicbot.json not found in the archive");
}
...
async function extractTarGzBuffer(buffer: Buffer, destDir: string): Promise<void> {
  const tmpTar = path.join(os.tmpdir(), `atomicbot-tgz-${randomBytes(8).toString("hex")}.tar.gz`);
...
/**
 * Read the backup's atomicbot.json and derive the old stateDir
...
export function registerBackupHandlers(params: RegisterParams) {
  ...
  /**
   * Shared restore logic: given a source directory that contains atomicbot.json,
  ...
  ipcMain.handle("backup-restore", async (_evt, p: { data?: unknown; filename?: unknown }) => {
    ...
    const tmpDir = path.join(os.tmpdir(), `atomicbot-restore-${randomBytes(8).toString("hex")}`);
  ...
  // ── Detect local AtomicBot instance at ~/.atomicbot ─────────────────────
  ipcMain.handle("backup-detect-local", async () => {
    try {
      const atomicbotDir = path.join(os.homedir(), ".atomicbot");
      const configPath = path.join(atomicbotDir, "atomicbot.json");
      const exists = fs.existsSync(configPath);
      return { found: exists, path: atomicbotDir };
    } catch (err) {
  ...
  ipcMain.handle("backup-restore-from-dir", async (_evt, p: { dirPath?: unknown }) => {
    ...
    try {
      // Validate the directory contains atomicbot.json
      const configPath = path.join(dirPath, "atomicbot.json");
      if (!fs.existsSync(configPath)) {
        return {
          ok: false,
          error: "Invalid AtomicBot directory: atomicbot.json not found",
        };
      }
  ...
  // ── Open folder picker and validate it contains atomicbot.json ─────────
  ipcMain.handle("backup-select-folder", async () => {
    try {
      const parentWindow = getMainWindow();
      const dialogOpts = {
        title: "Select AtomicBot Configuration Folder",
      ...
      const configPath = path.join(selectedDir, "atomicbot.json");
      if (!fs.existsSync(configPath)) {
        return {
          ok: false,
          error:
            "Selected folder does not contain atomicbot.json. Please select a valid AtomicBot configuration directory.",
        };
      }
