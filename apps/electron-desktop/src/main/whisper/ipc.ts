...
async function downloadFile(
  ...
  const headers: Record<string, string> = {
    "User-Agent": opts?.userAgent ?? "atomicbot-electron-desktop",
  };
...
export function registerWhisperIpcHandlers(params: {
  ...
  ipcMain.handle("whisper-model-download", async (_evt, p?: { model?: string }) => {
    ...
    try {
      ...
      await downloadFile(model.url, modelPath, {
        ...
        userAgent: "atomicbot-electron-desktop/whisper-model-download",
        signal: abort.signal,
      });
  ...
  ipcMain.handle(
    ...
    async (_evt, p: { audio?: string; language?: string; model?: string }) => {
      ...
      const tmpDir = path.join(os.tmpdir(), "atomicbot-whisper");
      ensureDir(tmpDir);
