...
describe("inferWorkspaceDirFromConfigPath", () => {
  it("infers workspace dir from config path", () => {
    expect(inferWorkspaceDirFromConfigPath("/home/user/.atomicbot/config.yaml")).toBe(
      "/home/user/.atomicbot/workspace"
    );
  });

  it("returns fallback for undefined", () => {
    expect(inferWorkspaceDirFromConfigPath(undefined)).toBe("~/atomicbot-workspace");
  });

  it("returns fallback for empty string", () => {
    expect(inferWorkspaceDirFromConfigPath("")).toBe("~/atomicbot-workspace");
    expect(inferWorkspaceDirFromConfigPath("  ")).toBe("~/atomicbot-workspace");
  });

  it("handles Windows paths", () => {
    expect(inferWorkspaceDirFromConfigPath("C:\\Users\\me\\.atomicbot\\config.yaml")).toBe(
      "C:\\Users\\me\\.atomicbot\\workspace"
    );
  });

  it("returns fallback for path with no separator", () => {
    expect(inferWorkspaceDirFromConfigPath("config.yaml")).toBe("~/atomicbot-workspace");
  });
});
