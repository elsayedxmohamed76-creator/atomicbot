/**
 * Typed wrapper around `window.atomicbotDesktop` — the Electron preload
 ...
 * `window.atomicbotDesktop` directly.  This centralises the availability
...
export type DesktopApi = NonNullable<Window["atomicbotDesktop"]>;
...
export function getDesktopApi(): DesktopApi {
  const api = window.atomicbotDesktop;
  if (!api) {
...
export function getDesktopApiOrNull(): DesktopApi | null {
  return window.atomicbotDesktop ?? null;
}
