...
} from "./lib/atomicbot-bundle-config.mjs";
...
import {
  ...
  tryRemove,
  uniqSortedStrings,
} from "./lib/atomicbot-bundle-utils.mjs";
...
const skipVerify = /^(1|true|yes)$/i.test(
  String(process.env.ATOMICBOT_BUNDLE_SKIP_VERIFY || "").trim()
);
const persistGeneratedExternals = /^(1|true|yes)$/i.test(
  String(process.env.ATOMICBOT_BUNDLE_WRITE_GENERATED_EXTERNALS || "").trim()
);
const strictGeneratedExternals = /^(1|true|yes)$/i.test(
  String(process.env.ATOMICBOT_BUNDLE_STRICT_EXTERNALS || "").trim()
);
console.log(`[electron-desktop] prepare-atomicbot-bundle safe mode: ${safeMode ? "on" : "off"}`);
...
async function main() {
  ...
  run(PNPM, ["-C", repoRoot, "--filter", "atomicbot", "--prod", "--legacy", "deploy", outDir]);
  ...
  if (persistGeneratedExternals && learnedAdaptiveExternals.size > 0) {
    ...
    console.log(
      "[electron-desktop] To persist them, rerun with ATOMICBOT_BUNDLE_WRITE_GENERATED_EXTERNALS=1"
    );
  }
  ...
  if (strictGeneratedExternals && learnedAdaptiveExternals.size > 0) {
    throw new Error(
      [
        ...
        "Run: npm run refresh:atomicbot-externals",
        "Then commit scripts/lib/atomicbot-bundle-generated-externals.json",
      ].join("\n")
    );
  }
  ...
  if (!skipVerify) {
    ...
    console.log("[electron-desktop] Bundle verification skipped by ATOMICBOT_BUNDLE_SKIP_VERIFY");
  }

  console.log(`[electron-desktop] AtomicBot bundle prepared at: ${outDir}`);
}
