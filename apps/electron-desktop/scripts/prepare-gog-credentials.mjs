...
function readSourceJson() {
  const fromPath =
    (process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_PATH &&
      String(process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_PATH).trim()) ||
  ...
  const fromB64Raw =
    (process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_B64 &&
      String(process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_B64).trim()) ||
    (process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_BASE64 &&
      String(process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_BASE64).trim()) ||
  ...
  const fromJson =
    (process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_JSON &&
      String(process.env.ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_JSON)) ||
  ...
  throw new Error(
    [
      ...
      "Set one of:",
      "- ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_PATH=/path/to/client_secret.json",
      "- ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_B64='<base64>'",
      "- ATOMICBOT_GOG_OAUTH_CLIENT_SECRET_JSON='{...json...}'",
    ].join("\n")
  );
}
