...
import { CRITICAL_RUNTIME_PACKAGES, SMOKE_COMMANDS } from "./atomicbot-bundle-config.mjs";
...
function assertExists(filePath, label) {
  if (!fs.existsSync(filePath)) {
    throw new Error(`[verify-atomicbot-bundle] Missing ${label}: ${filePath}`);
  }
}

export function verifyBundle(params) {
  const { outDir, timeoutMs = 20_000 } = params;
  const atomicbotEntry = path.join(outDir, "atomicbot.mjs");
  ...
  assertExists(atomicbotEntry, "atomicbot entrypoint");
...
`;
  const resolveRes = runCheck(
    "node",
    ["-e", resolveScript, atomicbotEntry, ...CRITICAL_RUNTIME_PACKAGES],
    { cwd: outDir, timeout: timeoutMs }
  );
  if (resolveRes.status !== 0) {
    throw new Error(
      `[verify-atomicbot-bundle] package resolve check failed.\n${resolveRes.stderr || resolveRes.stdout}`
    );
  }

  for (const cmdArgs of SMOKE_COMMANDS) {
    const smokeRes = runCheck("node", [atomicbotEntry, ...cmdArgs], {
      cwd: outDir,
      timeout: timeoutMs,
    });
    if (smokeRes.status !== 0) {
      throw new Error(
        `[verify-atomicbot-bundle] smoke failed: node atomicbot.mjs ${cmdArgs.join(" ")}\n${
