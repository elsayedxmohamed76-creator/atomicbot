...
test.describe("Backup roundtrip (create -> mutate -> restore -> verify)", () => {
  ...
  test("copy stateDir and create ZIP backup", async () => {
    test.setTimeout(15_000);
    const atomicbotDir = path.join(ctx.userDataDir, "atomicbot");
    const copyDest = path.join(tempBackupDir, "atomicbot");
    fs.cpSync(atomicbotDir, copyDest, { recursive: true });

    // Verify the copy contains atomicbot.json
    expect(fs.existsSync(path.join(copyDest, "atomicbot.json"))).toBe(true);
