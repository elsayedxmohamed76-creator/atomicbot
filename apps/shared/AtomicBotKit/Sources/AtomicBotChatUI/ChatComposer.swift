...
@MainActor
struct AtomicBotChatComposer: View {
    @Bindable var viewModel: AtomicBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AtomicBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AtomicBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AtomicBotPendingAttachment.id)
                { (att: AtomicBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AtomicBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(AtomicBotChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AtomicBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AtomicBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AtomicBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
