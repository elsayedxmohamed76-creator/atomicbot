...
private let chatUILogger = Logger(subsystem: "ai.atomicbot", category: "AtomicBotChatUI")

@MainActor
@Observable
public final class AtomicBotChatViewModel {
    public private(set) var messages: [AtomicBotChatMessage] = []
    ...
    public var attachments: [AtomicBotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AtomicBotChatPendingToolCall] = []
    public private(set) var sessions: [AtomicBotChatSessionEntry] = []
    private let transport: any AtomicBotChatTransport
    ...
    private var pendingToolCallsById: [String: AtomicBotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AtomicBotChatTransport) {
    ...
    public var sessionChoices: [AtomicBotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [AtomicBotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AtomicBotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AtomicBotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AtomicBotChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AtomicBotChatMessage) -> AtomicBotChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AtomicBotChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AtomicBotChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: AtomicBotChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [AtomicBotChatMessage],
        incoming: [AtomicBotChatMessage]) -> [AtomicBotChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AtomicBotChatMessage]) -> [AtomicBotChatMessage] {
        var result: [AtomicBotChatMessage] = []
    ...
    private static func dedupeKey(for message: AtomicBotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AtomicBotChatMessageContent] = [
            AtomicBotChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> AtomicBotChatAttachmentPayload in
            AtomicBotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AtomicBotChatSessionEntry {
        AtomicBotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AtomicBotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AtomicBotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AtomicBotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AtomicBotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AtomicBotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
