...
enum AtomicBotChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("AtomicBotChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("AtomicBotChatTheme.onboardingAssistantBubble"),
...
enum AtomicBotPlatformImageFactory {
    static func image(_ image: AtomicBotPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
