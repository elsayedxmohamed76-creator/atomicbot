...
@MainActor
public struct AtomicBotChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: AtomicBotChatViewModel
    ...
    public init(
        viewModel: AtomicBotChatViewModel,
    ...
    private var visibleMessages: [AtomicBotChatMessage] {
        let base: [AtomicBotChatMessage]
    ...
    private func mergeToolResults(in messages: [AtomicBotChatMessage]) -> [AtomicBotChatMessage] {
        ...
        for message in messages {
            ...
            let merged = AtomicBotChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: AtomicBotChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: AtomicBotChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: AtomicBotChatMessage) -> String {
