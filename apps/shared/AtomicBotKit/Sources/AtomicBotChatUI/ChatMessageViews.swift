...
struct ChatMessageBubble: View {
    let message: AtomicBotChatMessage
    let style: AtomicBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AtomicBotChatTheme.userText : AtomicBotChatTheme.assistantText
    ...
    private var inlineAttachments: [AtomicBotChatMessageContent] {
    ...
    private var toolCalls: [AtomicBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AtomicBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AtomicBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AtomicBotChatTheme.onboardingAssistantBubble
        }
        return AtomicBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AtomicBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AtomicBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AtomicBotChatTheme.userText : AtomicBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AtomicBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AtomicBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AtomicBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AtomicBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AtomicBotChatTheme.assistantText)
            }
        }
    }
}
