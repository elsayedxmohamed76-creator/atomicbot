...
import Foundation
import Testing
@testable import AtomicBotChatUI
...
private final class TestChatTransport: @unchecked Sendable, AtomicBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AtomicBotChatHistoryPayload]
    private let sessionsResponses: [AtomicBotChatSessionsListResponse]

    private let stream: AsyncStream<AtomicBotChatTransportEvent>
    private let continuation: AsyncStream<AtomicBotChatTransportEvent>.Continuation

    init(
        historyResponses: [AtomicBotChatHistoryPayload],
        sessionsResponses: [AtomicBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AtomicBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AtomicBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AtomicBotChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [AtomicBotChatAttachmentPayload]) async throws -> AtomicBotChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return AtomicBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AtomicBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AtomicBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AtomicBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = AtomicBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = AtomicBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { AtomicBotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = AtomicBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = AtomicBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = AtomicBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { AtomicBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
