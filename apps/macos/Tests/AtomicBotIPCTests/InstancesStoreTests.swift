...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AtomicBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AtomicBotProtocol.AnyCodable] = [
            "presence": .init([AtomicBotProtocol.AnyCodable(entry)]),
        ]
        let payload = AtomicBotProtocol.AnyCodable(payloadMap)
