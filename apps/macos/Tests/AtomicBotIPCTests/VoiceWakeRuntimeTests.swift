...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["atomicbot"]
        let text = "hey atomicbot"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey atomicbot do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["atomicbot"], minPostTriggerGap: 0.3)
