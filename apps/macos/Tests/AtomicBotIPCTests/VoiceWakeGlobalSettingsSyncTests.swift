...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = AtomicBotProtocol.AnyCodable(["triggers": ["atomicbot", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["atomicbot", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = AtomicBotProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
