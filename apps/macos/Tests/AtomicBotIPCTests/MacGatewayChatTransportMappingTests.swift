...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AtomicBotProtocol.AnyCodable(["ok": AtomicBotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AtomicBotProtocol.AnyCodable(["ok": AtomicBotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AtomicBotProtocol.AnyCodable([
            "runId": AtomicBotProtocol.AnyCodable("run-1"),
            "sessionKey": AtomicBotProtocol.AnyCodable("main"),
            "state": AtomicBotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AtomicBotProtocol.AnyCodable(["a": AtomicBotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
