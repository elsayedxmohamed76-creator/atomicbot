...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AtomicBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.atomicbot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AtomicBotKit.AnyCodable]()) {
                $0[$1.key] = AtomicBotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AtomicBotProtocol.AnyCodable?) -> [String: AtomicBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AtomicBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AtomicBotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AtomicBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("atomicbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("atomicbot.control.agent")
}
