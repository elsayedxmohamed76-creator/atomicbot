...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: AtomicBotOAuthStore.AnthropicOAuthStatus = AtomicBotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: AtomicBotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = AtomicBotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = AtomicBotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try AtomicBotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. AtomicBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: AtomicBotOAuthStore.AnthropicOAuthStatus,
