...
enum DebugActions {
    private static let verboseDefaultsKey = "atomicbot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "atomicbot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AtomicBotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AtomicBot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AtomicBotPaths.configURL
        guard
