...
enum CommandResolver {
    private static let projectRootDefaultsKey = "atomicbot.gatewayProjectRootPath"
    private static let helperName = "atomicbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let atomicbotEntry = root.appendingPathComponent("atomicbot.mjs").path
        if FileManager().isReadableFile(atPath: atomicbotEntry) { return atomicbotEntry }
        let binEntry = root.appendingPathComponent("bin/atomicbot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/atomicbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let atomicbotPaths = self.atomicbotManagedPaths(home: home)
        if !atomicbotPaths.isEmpty {
            extras.insert(contentsOf: atomicbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + atomicbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func atomicbotManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".atomicbot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func atomicbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAtomicBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("atomicbot.mjs").path,
            root.appendingPathComponent("bin/atomicbot.js").path,
        ]
    ...
    static func hasAnyAtomicBotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.atomicbotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func atomicbotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let atomicbotPath = self.projectAtomicBotExecutable(projectRoot: root) {
                return [atomicbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "atomicbot", subcommand] + extraArgs
            }
            if let atomicbotPath = self.atomicbotExecutable(searchPaths: searchPaths) {
                return [atomicbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            atomicbot entrypoint missing (looked for dist/index.js or atomicbot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real atomicbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/atomicbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v atomicbot >/dev/null 2>&1; then
          CLI="$(command -v atomicbot)"
          atomicbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/atomicbot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/atomicbot.mjs"
            node "$PRJ/atomicbot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/atomicbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/atomicbot.js"
            node "$PRJ/bin/atomicbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent atomicbot"
          pnpm --silent atomicbot \(quotedArgs);
        else
          echo "atomicbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AtomicBotConfigFile.loadDict()
