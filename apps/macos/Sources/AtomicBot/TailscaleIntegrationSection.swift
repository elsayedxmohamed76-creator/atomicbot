...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.atomicbot/atomicbot.json. Prefer ATOMICBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.atomicbot/atomicbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.atomicbot/atomicbot.json. Restart the gateway to apply."
        }
