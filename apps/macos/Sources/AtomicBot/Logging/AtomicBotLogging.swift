...
enum AtomicBotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = AtomicBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AtomicBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AtomicBotLogging.bootstrapIfNeeded()
        let label = AtomicBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AtomicBotOSLogHandler: LogHandler {
...
struct AtomicBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AtomicBotLogging.parseLabel(self.label)
