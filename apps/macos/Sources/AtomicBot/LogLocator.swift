...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ATOMICBOT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/atomicbot")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("atomicbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("atomicbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/atomicbot/ (rolling or stdout), or nil if none exist.
