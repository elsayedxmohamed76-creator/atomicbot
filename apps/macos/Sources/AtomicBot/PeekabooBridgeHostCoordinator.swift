...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.atomicbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: AtomicBotPeekabooBridgeServices?
    private static var atomicbotSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("AtomicBot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = AtomicBotPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.atomicbotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.atomicbotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class AtomicBotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.atomicbot.peekaboo")
