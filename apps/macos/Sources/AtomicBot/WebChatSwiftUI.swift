...
private let webChatSwiftLogger = Logger(subsystem: "ai.atomicbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AtomicBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AtomicBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AtomicBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AtomicBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AtomicBotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    AtomicBotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    AtomicBotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AtomicBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AtomicBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AtomicBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AtomicBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AtomicBotChatView>) -> NSViewController
    {
