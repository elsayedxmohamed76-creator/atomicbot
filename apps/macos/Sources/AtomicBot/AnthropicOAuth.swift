...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (AtomicBot token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: AtomicBotOAuthStore.AnthropicOAuthStatus = AtomicBotOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.atomicbot", category: "anthropic-oauth")
...
enum AtomicBotOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let atomicbotOAuthDirEnv = "ATOMICBOT_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "AtomicBot OAuth token file not found"
            case .unreadableFile: "AtomicBot OAuth token file not readable"
            case .invalidJSON: "AtomicBot OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in AtomicBot OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "AtomicBot OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.atomicbotOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".atomicbot", isDirectory: true)
