...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.atomicbot.mac`) even if the shipped
    ...
    {
        if environment["ATOMICBOT_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "atomicbot.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "atomicbot.nixMode") { return true }
