...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.atomicbot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AtomicBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AtomicBotSessionPreviewEntry,
    ...
    private static func cache(payload: AtomicBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AtomicBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AtomicBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AtomicBotKit.AnyCodable]) -> [AtomicBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AtomicBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AtomicBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AtomicBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AtomicBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AtomicBotChatMessage) -> String? {
