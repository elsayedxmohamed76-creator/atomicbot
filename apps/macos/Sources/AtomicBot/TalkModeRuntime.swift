...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.atomicbot", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.atomicbot", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [AtomicBotChatMessage] = messages.compactMap { item in
