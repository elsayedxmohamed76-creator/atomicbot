...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ATOMICBOT_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AtomicBotTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AtomicBotTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AtomicBotTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AtomicBotTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AtomicBotTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AtomicBotTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AtomicBotTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AtomicBotTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AtomicBotTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AtomicBotTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: AtomicBotAgentEventPayload.self) else {
                continue
            }
