...
final class NodeAppModel {
    private let deepLinkLogger = Logger(subsystem: "ai.atomicbot.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.atomicbot.ios", category: "PushWake")
    private let locationWakeLogger = Logger(subsystem: "ai.atomicbot.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.atomicbot.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = AtomicBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = AtomicBotCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = AtomicBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = AtomicBotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: AtomicBotLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(AtomicBotGatewayHealthOK.self, from: data) else {
                        return false
                    }
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for atomicbot:// links.)
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: AtomicBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: AtomicBotNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AtomicBotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(AtomicBotLocationGetParams.self, from: req.paramsJSON)) ??
            AtomicBotLocationGetParams()
        ...
        let payload = AtomicBotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(AtomicBotCanvasPresentParams.self, from: req.paramsJSON)) ??
                AtomicBotCanvasPresentParams()
        ...
        case AtomicBotCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case AtomicBotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(AtomicBotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case AtomicBotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(AtomicBotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case AtomicBotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(AtomicBotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case AtomicBotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.atomicbotA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing atomicbotA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotCanvasA2UICommand.push.rawValue, AtomicBotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AtomicBotKit.AnyCodable]
            if command == AtomicBotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(AtomicBotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try AtomicBotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(AtomicBotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try AtomicBotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.atomicbotA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case AtomicBotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(AtomicBotCameraSnapParams.self, from: req.paramsJSON)) ??
                AtomicBotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case AtomicBotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(AtomicBotCameraClipParams.self, from: req.paramsJSON)) ??
                AtomicBotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(AtomicBotScreenRecordParams.self, from: req.paramsJSON)) ??
            AtomicBotScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AtomicBotSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AtomicBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AtomicBotNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AtomicBotNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AtomicBotChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AtomicBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = AtomicBotChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(AtomicBotPhotosLatestParams.self, from: req.paramsJSON)) ??
            AtomicBotPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(AtomicBotContactsSearchParams.self, from: req.paramsJSON)) ??
                AtomicBotContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotContactsCommand.add.rawValue:
            let params = try Self.decodeParams(AtomicBotContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(AtomicBotCalendarEventsParams.self, from: req.paramsJSON)) ??
                AtomicBotCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(AtomicBotCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(AtomicBotRemindersListParams.self, from: req.paramsJSON)) ??
                AtomicBotRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(AtomicBotRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(AtomicBotMotionActivityParams.self, from: req.paramsJSON)) ??
                AtomicBotMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(AtomicBotPedometerParams.self, from: req.paramsJSON)) ??
                AtomicBotPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([AtomicBotLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([
            ...
            AtomicBotCanvasCommand.hide.rawValue,
            AtomicBotCanvasCommand.navigate.rawValue,
            AtomicBotCanvasCommand.evalJS.rawValue,
        ...
        register([
            ...
            AtomicBotCanvasA2UICommand.push.rawValue,
            AtomicBotCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AtomicBotCameraCommand.list.rawValue,
            AtomicBotCameraCommand.snap.rawValue,
            AtomicBotCameraCommand.clip.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleCameraInvoke(req)
        }

        register([AtomicBotScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([AtomicBotSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([AtomicBotChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleChatPushInvoke(req)
        }

        register([
            AtomicBotDeviceCommand.status.rawValue,
            AtomicBotDeviceCommand.info.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AtomicBotWatchCommand.status.rawValue,
            AtomicBotWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([AtomicBotPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handlePhotosInvoke(req)
        }

        register([
            AtomicBotContactsCommand.search.rawValue,
            AtomicBotContactsCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AtomicBotCalendarCommand.events.rawValue,
            AtomicBotCalendarCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AtomicBotRemindersCommand.list.rawValue,
            AtomicBotRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AtomicBotMotionCommand.activity.rawValue,
            AtomicBotMotionCommand.pedometer.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            AtomicBotTalkCommand.pttStart.rawValue,
            AtomicBotTalkCommand.pttStop.rawValue,
            AtomicBotTalkCommand.pttCancel.rawValue,
            AtomicBotTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case AtomicBotWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = AtomicBotWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case AtomicBotWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(AtomicBotWatchNotifyParams.self, from: req.paramsJSON)
            let title = params.title.trimmingCharacters(in: .whitespacesAndNewlines)
            let body = params.body.trimmingCharacters(in: .whitespacesAndNewlines)
            if title.isEmpty && body.isEmpty {
                return BridgeInvokeResponse(
            ...
            do {
                ...
                let payload = AtomicBotWatchNotifyPayload(
    ...
    func locationMode() -> AtomicBotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return AtomicBotLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                        ...
                        await MainActor.run {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). Approve on gateway and return to AtomicBot."
                            } else {
                                self.gatewayStatusText = "Pairing required. Approve on gateway and return to AtomicBot."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "atomicbot-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "AtomicBot Share Self-Test",
            url: URL(string: "https://atomicbot.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.atomicbotPushKind(userInfo)
        self.pushWakeLogger.info(
    ...
    private static func atomicbotPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["atomicbot"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["atomicbot"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
