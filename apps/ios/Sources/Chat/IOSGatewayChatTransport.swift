...
struct IOSGatewayChatTransport: AtomicBotChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.atomicbot", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AtomicBotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AtomicBotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AtomicBotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AtomicBotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AtomicBotChatAttachmentPayload]) async throws -> AtomicBotChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AtomicBotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AtomicBotChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AtomicBotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AtomicBotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AtomicBotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AtomicBotAgentEventPayload.self)
                        {
