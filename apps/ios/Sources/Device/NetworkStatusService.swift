...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> AtomicBotNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> AtomicBotNetworkStatusPayload {
        let status: AtomicBotNetworkPathStatus = switch path.status {
        ...
        var interfaces: [AtomicBotNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return AtomicBotNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> AtomicBotNetworkStatusPayload {
        AtomicBotNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
