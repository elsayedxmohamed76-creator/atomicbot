...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [AtomicBotCapability.canvas.rawValue, AtomicBotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(AtomicBotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(AtomicBotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = AtomicBotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(AtomicBotCapability.location.rawValue) }

        caps.append(AtomicBotCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(AtomicBotCapability.watch.rawValue)
        }
        caps.append(AtomicBotCapability.photos.rawValue)
        caps.append(AtomicBotCapability.contacts.rawValue)
        caps.append(AtomicBotCapability.calendar.rawValue)
        caps.append(AtomicBotCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(AtomicBotCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            AtomicBotScreenCommand.record.rawValue,
            AtomicBotSystemCommand.notify.rawValue,
            AtomicBotChatCommand.push.rawValue,
        ...
        if caps.contains(AtomicBotCapability.camera.rawValue) {
            commands.append(AtomicBotCameraCommand.list.rawValue)
            commands.append(AtomicBotCameraCommand.snap.rawValue)
            commands.append(AtomicBotCameraCommand.clip.rawValue)
        }
        if caps.contains(AtomicBotCapability.location.rawValue) {
            commands.append(AtomicBotLocationCommand.get.rawValue)
        }
        if caps.contains(AtomicBotCapability.device.rawValue) {
            commands.append(AtomicBotDeviceCommand.status.rawValue)
            commands.append(AtomicBotDeviceCommand.info.rawValue)
        }
        if caps.contains(AtomicBotCapability.watch.rawValue) {
            commands.append(AtomicBotWatchCommand.status.rawValue)
            commands.append(AtomicBotWatchCommand.notify.rawValue)
        }
        if caps.contains(AtomicBotCapability.photos.rawValue) {
            commands.append(AtomicBotPhotosCommand.latest.rawValue)
        }
        if caps.contains(AtomicBotCapability.contacts.rawValue) {
            commands.append(AtomicBotContactsCommand.search.rawValue)
            commands.append(AtomicBotContactsCommand.add.rawValue)
        }
        if caps.contains(AtomicBotCapability.calendar.rawValue) {
            commands.append(AtomicBotCalendarCommand.events.rawValue)
            commands.append(AtomicBotCalendarCommand.add.rawValue)
        }
        if caps.contains(AtomicBotCapability.reminders.rawValue) {
            commands.append(AtomicBotRemindersCommand.list.rawValue)
            commands.append(AtomicBotRemindersCommand.add.rawValue)
        }
        if caps.contains(AtomicBotCapability.motion.rawValue) {
            commands.append(AtomicBotMotionCommand.activity.rawValue)
            commands.append(AtomicBotMotionCommand.pedometer.rawValue)
        }
