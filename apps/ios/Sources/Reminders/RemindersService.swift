...
final class RemindersService: RemindersServicing {
    func list(params: AtomicBotRemindersListParams) async throws -> AtomicBotRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AtomicBotReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return AtomicBotReminderPayload(
        ...
        return AtomicBotRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AtomicBotRemindersAddParams) async throws -> AtomicBotRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AtomicBotReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AtomicBotRemindersAddPayload(reminder: payload)
    }
