...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AtomicBotContactsSearchParams) async throws -> AtomicBotContactsSearchPayload {
        ...
        return AtomicBotContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AtomicBotContactsAddParams) async throws -> AtomicBotContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AtomicBotContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AtomicBotContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AtomicBotContactPayload {
        AtomicBotContactPayload(
