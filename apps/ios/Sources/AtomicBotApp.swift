...
@MainActor
final class AtomicBotAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.atomicbot.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.atomicbot.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.atomicbot.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "atomicbot.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "atomicbot.watch.promptId"
    static let sessionKeyKey = "atomicbot.watch.sessionKey"
    static let actionPrimaryIDKey = "atomicbot.watch.action.primary.id"
    static let actionPrimaryLabelKey = "atomicbot.watch.action.primary.label"
    static let actionSecondaryIDKey = "atomicbot.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "atomicbot.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "atomicbot.watch.action.primary"
    static let actionSecondaryIdentifier = "atomicbot.watch.action.secondary"
    static let categoryPrefix = "atomicbot.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AtomicBotWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AtomicBotWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AtomicBotWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "AtomicBot" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: AtomicBotWatchAction,
        secondaryAction: AtomicBotWatchAction?) -> [UNNotificationAction]
    {
...
@main
struct AtomicBotApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AtomicBotAppDelegate.self) private var appDelegate
...
extension AtomicBotApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("AtomicBot: installing uncaught exception handler")
