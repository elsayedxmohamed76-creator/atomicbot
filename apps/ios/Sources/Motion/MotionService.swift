...
final class MotionService: MotionServicing {
    func activities(params: AtomicBotMotionActivityParams) async throws -> AtomicBotMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[AtomicBotMotionActivityEntry], Error>) in
        ...
        return AtomicBotMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AtomicBotPedometerParams) async throws -> AtomicBotPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<AtomicBotPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AtomicBotPedometerPayload(
