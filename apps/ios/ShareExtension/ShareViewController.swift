...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.atomicbot.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to AtomicBot", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to AtomicBot gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to AtomicBot.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AtomicBotShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "AtomicBot is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "AtomicBot Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("atomicbot-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
